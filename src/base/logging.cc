...
#if defined(OS_WEBOS) && defined(USE_PMLOG)
    base::CommandLine* command_line = base::CommandLine::ForCurrentProcess();
    if (command_line->HasSwitch(switches::kLogLevelToPmLog)) {
      int level = std::stoi(
          command_line->GetSwitchValueASCII(switches::kLogLevelToPmLog));
      if (severity_ >= level)
        RAW_PMLOG_INFO("chromium.log", "%s", str_newline.c_str());
    }
#endif
...
#if defined(OS_WEBOS) && defined(USE_PMLOG)
void RawPmLog(int level, const char* msgid, const char* format, ...) {
  static PmLogContext pmlog_context = 0;
  char buf[4096] = {0};

  if (!pmlog_context)
    PmLogGetContext("chromium.raw", &pmlog_context);

  va_list args;
  va_start(args, format);
  vsnprintf(buf, 4096 - 1, format, args);
  va_end(args);

  char *escapedStr = g_strescape(buf, NULL);
  if (level >= LOG_INFO)
    PmLogInfo(pmlog_context, msgid, 1, PMLOGKS("INFO", escapedStr), "");
  else
    PmLogDebug(pmlog_context, "%s", escapedStr);
  g_free(escapedStr);
}
#endif
