...
// use pmlog for debugging chromium code
// NOTE : if level is over INFO, use PmLogInfo.
//        other case(VERBOSE) use PmLogDebug.
BASE_EXPORT void RawPmLog(
    int level, const char* msgid, const char* format, ...);

#define RAW_PMLOG(level, msgid, ...) \
    logging::RawPmLog(logging::LOG_ ## level, msgid, __VA_ARGS__)
#define RAW_PMLOG_DEBUG(...) \
    RAW_PMLOG(VERBOSE, "", __VA_ARGS__)
#define RAW_PMLOG_INFO(msgid, ...) \
    RAW_PMLOG(INFO, msgid, __VA_ARGS__)
#else
#define RAW_PMLOG(level, msgid, ...) (void) 0
#define RAW_PMLOG_DEBUG(...) (void) 0
#define RAW_PMLOG_INFO(msgid, ...) (void) 0
#endif
}  // namespace logging
