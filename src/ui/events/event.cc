...
Event::Event(EventType type, base::TimeTicks time_stamp, int flags)
    : type_(type),
      ...
      event_source_type_(SOURCE_TYPE_UNKNOWN),
      source_device_id_(ED_UNKNOWN_DEVICE) {
  if (type_ < ET_LAST)
    name_ = EventTypeName(type_);
}

Event::Event(const base::NativeEvent& native_event, EventType type, int flags)
    : type_(type),
  ...
  event_source_type_ =
      static_cast<const Event*>(native_event)->event_source_type();
#endif
}
...
Event::Event(const Event& copy)
    : type_(copy.type_),
      ...
      phase_(EP_PREDISPATCH),
      result_(ER_UNHANDLED),
      event_source_type_(copy.event_source_type_),
...
MouseEvent::MouseEvent(EventType type,
  ...
  if (this->type() == ET_MOUSE_MOVED && IsAnyButton())
    SetType(ET_MOUSE_DRAGGED);
#if defined(OS_WEBOS)
  if (this->type() == ET_MOUSE_PRESSED || this->type() == ET_MOUSE_RELEASED)
    SetClickCount(GetRepeatCount(*this));
#endif
}
...
#if defined(OS_WEBOS)
  static const int kDoubleClickTimeMS = 1000;
  static const int kDoubleClickWidth = 10;
  static const int kDoubleClickHeight = 10;
#else
...
#endif
#if defined(OS_WEBOS)
  // RCU specific key events should contain DomKey according [GT-15908]
  if (code_ == DomCode::NONE)
    set_character(key_code_);
#endif
}
