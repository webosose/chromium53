...
#if defined(OS_WEBOS)
  DispatchKey(key, down, is_repeat, source_type, timestamp, device_id);
#else
...
#if defined(OS_WEBOS)
void KeyboardEvdev::TextInputModifier(uint32_t state, uint32_t modifier) {
  uint32_t keyboard_modifiers = modifiers_->GetModifierFlags();

  ui::EventType type = state ? ui::ET_KEY_PRESSED : ui::ET_KEY_RELEASED;
  switch (modifier) {
    case ui::EF_SHIFT_DOWN:
      if (type == ui::ET_KEY_PRESSED)
        keyboard_modifiers |= ui::EF_SHIFT_DOWN;
      else
        keyboard_modifiers &= ~ui::EF_SHIFT_DOWN;
      break;
    case ui::EF_CONTROL_DOWN:
      if (type == ui::ET_KEY_PRESSED)
        keyboard_modifiers |= ui::EF_CONTROL_DOWN;
      else
        keyboard_modifiers &= ~ui::EF_CONTROL_DOWN;
      break;
    case ui::EF_ALT_DOWN:
      if (type == ui::ET_KEY_PRESSED)
        keyboard_modifiers |= ui::EF_ALT_DOWN;
      else
        keyboard_modifiers &= ~ui::EF_ALT_DOWN;
      break;
    case ui::EF_CAPS_LOCK_ON:
      if (type == ui::ET_KEY_RELEASED)
        keyboard_modifiers ^= ui::EF_CAPS_LOCK_ON;
      break;
    default:
      keyboard_modifiers &= ~ui::EF_SHIFT_DOWN;
      keyboard_modifiers &= ~ui::EF_CONTROL_DOWN;
  ...
  modifiers_->SetModifierFlags(keyboard_modifiers);
}
