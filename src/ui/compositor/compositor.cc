...
Compositor::Compositor(ui::ContextFactory* context_factory,
  ...
  // TODO: should use cc::switches::kUIEnableAggressiveReleasePolicy
  //       and remove #include "content/public/common/content_switches.h"
  ...
  settings.renderer_settings.disable_opaque_shader_program =
      command_line->HasSwitch(switches::kUIDisableOpaqueShaderProgram);

  settings.renderer_settings.use_prepare_shader_program =
      command_line->HasSwitch(switches::kUIUsePrepareShaderProgram);
...
bool Compositor::IsVisible() const {
  return host_->visible();
}

void Compositor::PrepareOutputSurface() {
  if (host_->output_surface_lost()) {
    cc::SingleThreadProxy* proxy = static_cast<cc::SingleThreadProxy*>(host_->proxy());
