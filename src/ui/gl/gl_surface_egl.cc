...
void NativeViewGLSurfaceEGL::Destroy() {
  if (surface_) {
    ...
    RAW_PMLOG_INFO(
        "WebOSWebView",
        "egl window surface(this:%p,handle:%p) is destroyed. at %s(%d)", this,
        surface_, __FUNCTION__, __LINE__);
    surface_ = NULL;
  }
}
...
gfx::SwapResult NativeViewGLSurfaceEGL::SwapBuffers() {
  ...
  if (!eglSwapBuffers(GetDisplay(), surface_)) {
    ...
    std::string error = GetLastEGLErrorString();
    LOG(ERROR) << "eglSwapBuffers failed with error " << error.c_str()
               << "(display:" << GetDisplay() << ", handle:" << surface_ << ")";
    DVLOG(1) << "eglSwapBuffers failed with error " << error.c_str();
#else
...
#if defined(OS_WEBOS) && defined(USE_PMLOG)
    std::string error = GetLastEGLErrorString();
    LOG(ERROR) << "eglPostSubBufferNV failed with error " << error.c_str()
               << "(display:" << GetDisplay() << ", handle:" << surface_ << ")";
    DVLOG(1) << "eglPostSubBufferNV failed with error " << error.c_str();
#else
