...
bool CopyTextureCHROMIUMResourceManager::UnbindFramebufferTexture2D() {
  DCHECK(texture_target_ == GL_TEXTURE_2D ||
...
void CopyTextureCHROMIUMResourceManager::DoCopyTexture(
  ...
  if (source_target == GL_TEXTURE_2D && dest_target == GL_TEXTURE_2D &&
    ...
    DoCopyTexImage2D(decoder, source_target, source_id, dest_target, dest_id,
                     dest_internal_format, width, height);
    return;
  }
...
void CopyTextureCHROMIUMResourceManager::DoCopyTextureInternal(
  ...
  if (BindFramebufferTexture2D(dest_target, dest_id)) {
#ifndef NDEBUG
...
void CopyTextureCHROMIUMResourceManager::UnbindTexture(
    const gles2::GLES2Decoder* decoder,
    GLuint texture_id) {
  if (!initialized_) {
    DLOG(ERROR) << "CopyTextureCHROMIUM: Uninitialized manager.";
  ...
  if (texture_id_ != texture_id)
    return;

  UnbindFramebufferTexture2D();
