...
class GPU_EXPORT CopyTextureCHROMIUMResourceManager {
 public:
  ...
  // Unbind the given texture if it is bound.
  void UnbindTexture(const gles2::GLES2Decoder* decoder, GLuint texture_id);

  // The attributes used during invocation of the extension.
  static const GLuint kVertexPositionAttrib = 0;

 private:
  bool BindFramebufferTexture2D(GLenum target, GLuint texture_id);
  bool UnbindFramebufferTexture2D();

  void DoCopyTexImage2D(const gles2::GLES2Decoder* decoder,
                        GLenum source_target,
                        GLuint source_id,
                        GLenum dest_target,
                        GLuint dest_id,
