  ...
  for (GLsizei ii = 0; ii < n; ++ii) {
    ...
    if (texture_ref) {
      ...
      if (copy_texture_CHROMIUM_.get())
        copy_texture_CHROMIUM_->UnbindTexture(this, texture->service_id());
#if defined(OS_MACOSX)
...
bool GLES2DecoderImpl::MakeCurrent() {
  ...
  RAW_PMLOG_DEBUG(
      "chromium.gpu", "%s %s (surface:%p, handle:%p) at %s(%d)",
      "GLES2DecoderImpl: Context lost during MakeCurrent. EGL error",
      ui::GetLastEGLErrorString(), surface_.get(),
      surface_.get() ? surface_->GetHandle() : 0, __FUNCTION__, __LINE__);
...
error::Error GLES2DecoderImpl::HandleResizeCHROMIUM(
  ...
  if (is_offscreen) {
    ...
    if (!context_->IsCurrent(surface_.get())) {
      ...
      RAW_PMLOG_INFO(
          "chromium.gpu", "%s %s %s (surface:%p, handle:%p) at %s(%d)",
...
void GLES2DecoderImpl::DoGenerateMipmap(GLenum target) {
  ...
  // Workaround for Mali-4xx driver bug. If the glFinish is not executed
  // glGenerateMipmaps is failed and screen is displayed broken image.
  if (workarounds().force_complete_before_generating_mipmap)
    glFinish();
...
void GLES2DecoderImpl::DoLoseContextCHROMIUM(GLenum current, GLenum other) {
  RAW_PMLOG_INFO("chromium.gpu",
                 "DoLoseContextCHROMIUM current:%d other:%d at %s(%d)",
                 current, other, __FUNCTION__, __LINE__);
...
void GLES2DecoderImpl::OnContextLostError() {
  if (!WasContextLost()) {
    ...
    group_->LoseContexts(error::kUnknown);
    reset_by_robustness_extension_ = true;
    RAW_PMLOG_INFO("chromium.gpu", "LostContext at %s(%d)", __FUNCTION__,
                   __LINE__);
  }
}
