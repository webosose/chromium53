...
#if defined(OS_WEBOS) && defined(USE_WEBOS_SURFACE)
gfx::WebOSSurfaceHandle GpuChannelHost::ShareToGpuProcess(
    gfx::WebOSSurfaceHandle source_handle) {
  if (IsLost())
    return gfx::WebOSSurfaceHandle();

  gfx::WebOSSurfaceHandle handle;
  handle.data = source_handle.data;

  for (size_t i = 0; i < source_handle.descriptors.size(); i++) {
    int duped_fd = HANDLE_EINTR(dup(source_handle.descriptors[i].fd));
    if (duped_fd < 0) {
      // Need to close duped descriptors.
      for (size_t j = 0; j < i; j++)
        close(handle.descriptors[j].fd);
      return gfx::WebOSSurfaceHandle();
    }

    handle.descriptors.push_back(base::FileDescriptor(duped_fd, true));
  }
...
gfx::GpuMemoryBufferHandle GpuChannelHost::ShareGpuMemoryBufferToGpuProcess(
  ...
  switch (source_handle.type) {
    ...
    case gfx::WEBOS_SURFACE_BUFFER: {
      ...
      handle.webos_surface_handle =
          ShareToGpuProcess(source_handle.webos_surface_handle);
      *requires_sync_point = false;
      return handle;
    }
#endif
