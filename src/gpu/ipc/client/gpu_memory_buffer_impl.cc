...
// Returns a handle to the webOS surface that can be sent via IPC to the
// GPU process. The caller is responsible for ensuring it is closed. Returns
// an invalid handle on failure.
gfx::WebOSSurfaceHandle ShareToGpuProcess(
...
std::unique_ptr<GpuMemoryBufferImpl> GpuMemoryBufferImpl::CreateFromHandle(
  ...
  switch (handle.type) {
    ...
    case gfx::WEBOS_SURFACE_BUFFER:
      return GpuMemoryBufferImplWebOSSurface::CreateFromHandle(
          handle, size, format, usage, callback);
#endif
#if defined(OS_ANDROID)
