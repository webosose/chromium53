...
# since gyp_chromium is automatically forcing its inclusion.
{
  ...
  'variables': {
    ...
    'variables': {
      ...
      'arm_override_arch%': '<(arm_override_arch)',
      'sysroot%': '<(sysroot)',
      'use_sysroot%': '<(use_sysroot)',
      ...
      'posix_use_system_cross_toolchain%': 1,
    ...
    'posix_use_system_cross_toolchain%': '<(posix_use_system_cross_toolchain)',
  ...
  'conditions': [
    ...
    ['webos==1 and use_chromium_cbe!=0', {
      'target_defaults': {
       'ldflags': [
    ...
    ['OS=="linux" and target_arch=="arm" and host_arch!="arm" and chromeos==0 and clang==0 and posix_use_system_cross_toolchain==1', {
    ...
    ['OS=="linux" and target_arch=="arm" and host_arch!="arm" and chromeos==0 and clang==0 and posix_use_system_cross_toolchain==0', {
      # On WebOS build CC and CXX are overriden, so we are never going to use toolchain in path as specified
      # in previous test.
      'make_global_settings': [
        ['CC.host', '<(host_cc)'],
        ['CXX.host', '<(host_cxx)'],
