...

{
  ...
  'conditions': [
    ...
    ['webos!=0', {
      'targets': [
        {
          'target_name': 'PmLogLib',
          'type': 'none',
          'toolsets': ['target'],
          'direct_dependent_settings': {
            'cflags': [
              '<!@(<(pkg-config) --cflags PmLogLib-headers)'
            ],
          },
          'link_settings': {
            'libraries': [
              '<!@(<(pkg-config) --libs-only-l PmLogLib)',
            ],
          },
        },
        {
          'target_name': 'luna-service2',
          'type': 'none',
          'toolsets': ['target'],
          'direct_dependent_settings': {
            'cflags': [
          ...
          'link_settings': {
            'libraries': [
              '<!@(<(pkg-config) --libs-only-l luna-service2)',
            ],
          },
        },
        {
          'target_name': 'webossurface',
          'type': 'none',
          'toolsets': ['target'],
          'conditions': [
            ['use_webos_surface==1', {
              'direct_dependent_settings': {
                'cflags': [
                  '<!@(<(pkg-config) --cflags webossurface)'
                ],
              },
              'link_settings': {
                'ldflags': [
                  '<!@(<(pkg-config) --libs-only-L --libs-only-other webossurface)',
                ],
                'libraries': [
                  '<!@(<(pkg-config) --libs-only-l webossurface)',
                ],
              },
            }],
          ],
    ...
    ['use_umediaserver==1', {
      'targets': [
        {
          'target_name': 'umedia_api',
          'type': 'none',
          'toolsets': ['target'],
          'direct_dependent_settings': {
            'cflags': [
              '<!@(<(pkg-config) --cflags umedia_api)'
            ],
          },
          'link_settings': {
            'libraries': [
              '<!@(<(pkg-config) --libs-only-l umedia_api)',
            ],
          },
        },
        {
          'target_name': 'mdc_content_provider',
          'type': 'none',
          'conditions': [
            ['webos_tv!=0', {
              'toolsets': ['target'],
              'direct_dependent_settings': {
                'cflags': [
                  '<!@(<(pkg-config) --cflags mdc_content_provider)'
                ],
              },
              'link_settings': {
                'libraries': [
                  '<!@(<(pkg-config) --libs-only-l mdc_content_provider)',
                ],
              },
            }],
          ],
      ...
      'targets': [
        {
          ...
          'direct_dependent_settings': {
            'cflags': [
              '<!@(<(pkg-config) --cflags svp_inband_serializer)'
            ],
           },
           'link_settings': {
             'libraries': [
               '<!@(<(pkg-config) --libs-only-l svp_inband_serializer)',
             ],
           },
         },
       ],
    ...
    ['enable_lttng==1', {
      'targets': [
        ...
        },
        {
          ...
          'link_settings': {
            'ldflags': [
              '<!@(<(pkg-config) --libs-only-L --libs-only-other lttng-ust)',
            ],
            'libraries': [
