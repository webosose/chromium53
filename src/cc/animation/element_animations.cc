    ...
    if (animation_waiting_for_target->run_state() ==
      ...
      if (null_intersection) {
        ...
        if (base::CommandLine::ForCurrentProcess()->HasSwitch(
          ...
          animation_waiting_for_target->set_start_time_to_current_if_needed();
        }
#endif
        for (size_t j = animation_index + 1; j < animations_.size(); ++j) {
          if (animation_waiting_for_target->group() ==
            ...
            if (base::CommandLine::ForCurrentProcess()->HasSwitch(
                    switches::kEnableWebOSNativeScroll)) {
              animations_[j]->set_start_time_to_current_if_needed();
            }
#endif
          }
        }
      ...
      switch (animations_[i]->target_property()) {
        ...
        case TargetProperty::SCROLL_OFFSET: {
          ...
          NotifyClientScrollOffsetAnimated(
              ...
              animations_[i]->affects_pending_elements(), is_last_tick);
#else
          NotifyClientScrollOffsetAnimated(
              scroll_offset, animations_[i]->affects_active_elements(),
              animations_[i]->affects_pending_elements());
...
void ElementAnimations::NotifyClientScrollOffsetAnimated(
    ...
    bool notify_pending_elements,
    bool is_last_tick) {
  NotifyClientScrollOffsetAnimated(scroll_offset, notify_active_elements,
                                   notify_pending_elements);

  if (!base::CommandLine::ForCurrentProcess()->HasSwitch(
          switches::kEnableWebOSNativeScroll) || !animation_host())
    return;

  if (is_last_tick) {
    animation_host()
        ->mutator_host_client()
        ->WebOSClearCurrentlyScrollingLayer();
  } else {
    ...
    animation_host()->mutator_host_client()->WebOSSetCurrentlyScrollingElement(
        element_id());
  }
}
#endif
