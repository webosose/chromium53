...
// memory pressure.
static const int kMemoryPressureCacheSizeReductionFactor = 4;
static const int kMemoryPressureMaxItemsInCache = 100;
#endif
...
bool GpuImageDecodeController::CanFitSize(size_t size) const {
  ...
  if (memory_pressure_level_ ==
    ...
    bytes_limit = cached_bytes_limit_ / kMemoryPressureCacheSizeReductionFactor;
  }
#else
  bytes_limit = cached_bytes_limit_;
#endif
...
bool GpuImageDecodeController::ExceedsPreferredCount() const {
  ...
  if (memory_pressure_level_ ==
      base::MemoryPressureListener::MEMORY_PRESSURE_LEVEL_NONE) {
    items_limit = kMaxDiscardableItems;
  } else {
    items_limit = kMemoryPressureMaxItemsInCache;
  }
#else
  items_limit = cached_items_limit_;
#endif
