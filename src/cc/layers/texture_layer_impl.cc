...
TextureLayerImpl::TextureLayerImpl(LayerTreeImpl* tree_impl, int id)
    : LayerImpl(tree_impl, id),
      ...
      uses_punch_hole_for_video_(false),
      flipped_(true),
...
void TextureLayerImpl::PushPropertiesTo(LayerImpl* layer) {
  ...
  texture_layer->SetUsesPunchHoleForVideo(uses_punch_hole_for_video_);
...
void TextureLayerImpl::AppendQuads(RenderPass* render_pass,
  ...
  if (uses_punch_hole_for_video_) {
    ...
    solid_quad->SetAll(shared_quad_state, quad_rect, opaque_rect,
                       visible_quad_rect, false, SK_ColorTRANSPARENT, true);
  }
...
void TextureLayerImpl::EvictResources() {
  FreeTextureMailbox();
  texture_copy_.reset();
  external_texture_resource_ = 0;
  valid_texture_copy_ = false;
...
void TextureLayerImpl::SetUsesPunchHoleForVideo(
