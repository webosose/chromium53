...
void TextureLayer::SetUsesPunchHoleForVideo(bool uses_punch_hole_for_video) {
  if (uses_punch_hole_for_video_ == uses_punch_hole_for_video)
    return;
...
void TextureLayer::ReduceMemoryUsage() {
  if (layer_tree_host()->settings().use_aggressive_release_policy)
    holder_ref_.reset();
}
