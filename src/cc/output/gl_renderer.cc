...
void GLRenderer::DrawContentQuadNoAA(const DrawingFrame* frame,
  ...
  if (quad->ShouldDrawWithBlending() ||
      settings_->disable_opaque_shader_program) {
    if (quad->swizzle_contents) {
...
void GLRenderer::InitializeSharedObjects() {
  ...
  if (!settings_->use_prepare_shader_program)
    return;

  for (int i = 0; i < TexCoordPrecision::NUM_TEX_COORD_PRECISIONS; ++i) {
    for (int j = 0; j < SamplerType::NUM_SAMPLER_TYPES; ++j) {
      GetTileProgram((TexCoordPrecision)i, (SamplerType)j);
      GetTileProgramAA((TexCoordPrecision)i, (SamplerType)j);
    }

    GetRenderPassProgram((TexCoordPrecision)i, BlendMode::BLEND_MODE_NONE);
    GetRenderPassMaskProgramAA((TexCoordPrecision)i, SamplerType::SAMPLER_TYPE_2D, BlendMode::BLEND_MODE_NONE, false);
    GetRenderPassProgramAA((TexCoordPrecision)i, BlendMode::BLEND_MODE_NONE);

    GetTextureProgram((TexCoordPrecision)i, SamplerType::SAMPLER_TYPE_2D);
  }
}
