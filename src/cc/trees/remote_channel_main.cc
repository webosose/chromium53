...
void RemoteChannelMain::SetSkipFramesOnImpl(bool skip_frames) {
  TRACE_EVENT1("cc.remote", "RemoteChannelMain::SetSkipFramesOnImpl",
               "skip_frames", skip_frames);
  proto::CompositorMessage proto;
  proto::CompositorMessageToImpl* to_impl_proto = proto.mutable_to_impl();
  to_impl_proto->set_message_type(
      proto::CompositorMessageToImpl::SET_SKIP_FRAMES);
  proto::SetSkipFrames* skip_frames_message =
      to_impl_proto->mutable_skip_frames_message();
  skip_frames_message->set_skip_frames(skip_frames);

  VLOG(1) << "Sending skip frames: " << skip_frames << " to client.";
  SendMessageProto(proto);
}
