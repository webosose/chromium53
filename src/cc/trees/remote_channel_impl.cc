...
void RemoteChannelImpl::HandleProto(
  ...
  switch (proto.message_type()) {
    ...
    case proto::CompositorMessageToImpl::SET_SKIP_FRAMES: {
      const proto::SetSkipFrames& skip_frames_message =
          proto.skip_frames_message();
      bool skip_frames = skip_frames_message.skip_frames();
      VLOG(1) << "Received set skip frames to: " << skip_frames
              << " from the engine.";
      ImplThreadTaskRunner()->PostTask(
          FROM_HERE, base::Bind(&ProxyImpl::SetSkipFramesOnImpl,
                                proxy_impl_weak_ptr_, skip_frames));
    } break;
...
void RemoteChannelImpl::SetSkipFrames(bool skip_frames) {
  NOTREACHED() << "Should not be called on the remote client LayerTreeHost";
}
