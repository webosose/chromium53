...
#if defined(OS_WEBOS)
      memory_pressure_listener_(base::Bind(&LayerTreeHostImpl::OnMemoryPressure,
...
#if defined(OS_WEBOS)
  low_memory_policy_.bytes_limit_when_visible /= 2;
#endif
}
...
bool LayerTreeHostImpl::CanDraw() const {
  ...
  if (active_tree_->PicturesTexturesPurged()) {
    TRACE_EVENT_INSTANT0(
        "cc", "LayerTreeHostImpl::CanDraw pictures textures purged",
        TRACE_EVENT_SCOPE_THREAD);
    return false;
  }
...
void LayerTreeHostImpl::UpdateTileManagerMemoryPolicy(
  ...
  if (!visible_ && !use_gpu_rasterization_) {
    active_tree_->SetPicturesTexturesPurged();
    if (pending_tree_)
      pending_tree_->SetPicturesTexturesPurged();
    client_->SetNeedsCommitOnImplThread();
...
void LayerTreeHostImpl::EvictTreeResources() {
  EvictAllUIResources();

  if (!settings_.use_aggressive_release_policy)
    return;

  active_tree_->EvictResources();
  if (pending_tree_)
    pending_tree_->EvictResources();
  if (recycle_tree_)
    recycle_tree_->EvictResources();
}
...
InputHandler::ScrollStatus LayerTreeHostImpl::ScrollAnimatedBegin(
  ...
  if (scroll_node) {
    ...
    if (base::CommandLine::ForCurrentProcess()->HasSwitch(
        ...
        !animation_host_->HasPotentiallyRunningScrollAnimation(
            scroll_node->data.element_id, ElementListType::ACTIVE) &&
        !animation_host_->HasPotentiallyRunningScrollAnimation(
            scroll_node->data.element_id, ElementListType::PENDING)) {
      ClearCurrentlyScrollingLayer();
      scroll_node = nullptr;
    } else {
      gfx::Vector2dF delta;
...
#if defined(OS_WEBOS)
void LayerTreeHostImpl::WebOSClearCurrentlyScrollingLayer() {
  ClearCurrentlyScrollingLayer();
}

void LayerTreeHostImpl::WebOSSetCurrentlyScrollingElement(
    ElementId element_id) {
  DCHECK(active_tree_);
  LayerImpl* layer = active_tree_->LayerByElementId(element_id);
  if (layer) {
