...
void LayerTreeHost::FinishCommitOnImplThread(LayerTreeHostImpl* host_impl) {
  ...
  if (sync_tree->PicturesTexturesPurged())
    sync_tree->ResetPicturesTexturesPurged();
...
void LayerTreeHost::SetSkipFrames(bool skip_frames) {
  proxy_->SetSkipFrames(skip_frames);
}
