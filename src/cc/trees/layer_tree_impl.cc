...
LayerTreeImpl::LayerTreeImpl(
    ...
    : layer_tree_host_impl_(layer_tree_host_impl),
      ...
      pictures_textures_purged_(false),
...
void LayerTreeImpl::EvictResources() {
  if (!LayerListIsEmpty()) {
    LayerTreeHostCommon::CallFunctionForEveryLayer(
        this, [](LayerImpl* layer) { layer->EvictResources(); });
  }
}
...
void LayerTreeImpl::PushPropertiesTo(LayerTreeImpl* target_tree) {
  ...
  if (PicturesTexturesPurged())
    target_tree->SetPicturesTexturesPurged();
  else
    target_tree->ResetPicturesTexturesPurged();
...
bool LayerTreeImpl::PicturesTexturesPurged() const {
  return pictures_textures_purged_;
}

void LayerTreeImpl::SetPicturesTexturesPurged() {
  if (pictures_textures_purged_)
    return;
  pictures_textures_purged_ = true;
  layer_tree_host_impl_->OnCanDrawStateChangedForTree();
}

void LayerTreeImpl::ResetPicturesTexturesPurged() {
  if (!pictures_textures_purged_)
    return;
  pictures_textures_purged_ = false;
  layer_tree_host_impl_->OnCanDrawStateChangedForTree();
