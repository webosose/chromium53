...
#if defined(OS_WEBOS)
  // By this patch, defer_commits can be changed during RequestMainFrameUpdate.
  // So we need to block this commit if defer_commits was changed to true.
  if (defer_commits_) {
    TRACE_EVENT_INSTANT0("cc", "EarlyOut_DeferCommit",
                         TRACE_EVENT_SCOPE_THREAD);
    channel_main_->BeginMainFrameAbortedOnImpl(
...
void ProxyMain::FinishAllRendering() {
  DCHECK(IsMainThread());
  DCHECK(!defer_commits_ && !skip_frames_);
...
#if defined(OS_WEBOS)
  if (!defer_commits && commit_deferred_) {
    std::unique_ptr<BeginMainFrameAndCommitState> begin_main_frame_state(
        new BeginMainFrameAndCommitState());
    BeginMainFrame(std::move(begin_main_frame_state));
    commit_deferred_ = false;
  }
#endif
}

void ProxyMain::SetSkipFrames(bool skip_frames) {
  DCHECK(IsMainThread());
  ...
  if (skip_frames_)
    TRACE_EVENT_ASYNC_BEGIN0("cc", "ProxyMain::SetSkipFrames", this);
  else
    TRACE_EVENT_ASYNC_END0("cc", "ProxyMain::SetSkipFrames", this);

  channel_main_->SetSkipFramesOnImpl(skip_frames);
}
