...
StagingBufferPool::StagingBufferPool(base::SequencedTaskRunner* task_runner,
  ...
  task_runner_->PostTask(
      FROM_HERE, base::Bind(&StagingBufferPool::CreateMemoryPressureListener,
...
#if defined(OS_WEBOS)
void StagingBufferPool::CreateMemoryPressureListener() {
  memory_pressure_listener_.reset(new base::MemoryPressureListener(base::Bind(
      &StagingBufferPool::OnMemoryPressure, weak_ptr_factory_.GetWeakPtr())));
}
#endif

void StagingBufferPool::Shutdown() {
#if defined(OS_WEBOS)
  delete memory_pressure_listener_.release();
#endif
