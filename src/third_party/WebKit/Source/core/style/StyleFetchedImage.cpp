...
StyleFetchedImage::StyleFetchedImage(ImageResource* image, Document* document, const KURL& url)
    ...
    , m_url(url)
#if defined(OS_WEBOS)
    , m_commitDeferred(false)
#endif
{
    m_isImageResource = true;
    m_image->addClient(this);
#if defined(OS_WEBOS)
    if (!image->isLoaded() && url.protocolIs("file") && document && !UserGestureIndicator::processedUserGestureSinceLoad()) {
        m_commitDeferred = document->addDeferredBackgroundImage();
    }
#endif
...
#if defined(OS_WEBOS)
    if (m_commitDeferred) {
        m_document->removeDeferredBackgroundImage();
        m_commitDeferred = false;
    }
#endif
    m_document.clear();
