...
static CSSValue* consumeNavDirection(CSSParserTokenRange& range)
{
    // auto | <id> [ current | root | <target-name> ]? | inherit
    if (range.peek().id() == CSSValueAuto)
        return consumeIdent(range);

    if (range.peek().type() != HashToken || range.peek().getHashTokenType() != HashTokenId)
        return nullptr;

    CSSValue* parsedValue1 = CSSCustomIdentValue::create(range.consumeIncludingWhitespace().value().toAtomicString());
    if (range.peek().type() != IdentToken)
        return parsedValue1;

    CSSValue* parsedValue2 = consumeIdent<CSSValueCurrent, CSSValueRoot>(range);
    if (!parsedValue2)
        parsedValue2 = CSSCustomIdentValue::create(range.consumeIncludingWhitespace().value().toAtomicString());

    if (!parsedValue2)
        return parsedValue1;

    CSSValueList* values = CSSValueList::createSpaceSeparated();
    values->append(*parsedValue1);
    values->append(*parsedValue2);
...
CSSValue* CSSPropertyParser::parseSingleValue(CSSPropertyID unresolvedProperty, CSSPropertyID currentShorthand)
{
    ...
    case CSSPropertyNavUp:
        if (!RuntimeEnabledFeatures::cssNavigationEnabled())
            return nullptr;
        return consumeNavDirection(m_range);
