...
const CSSValue* ComputedStyleCSSValueMapping::get(CSSPropertyID propertyID, const ComputedStyle& style, const LayoutObject* layoutObject, Node* styledNode, bool allowVisitedStyle)
{
    ...
    case CSSPropertyNavIndex: {
        if (!RuntimeEnabledFeatures::cssNavigationEnabled())
            return nullptr;
        const StyleNavIndex* nav = style.navIndex();
        if (!nav || nav->isAuto)
            return CSSPrimitiveValue::createIdentifier(CSSValueAuto);
        return CSSPrimitiveValue::create(nav->idx, CSSPrimitiveValue::UnitType::Number);
    }
    ...
    case CSSPropertyNavUp: {
        if (!RuntimeEnabledFeatures::cssNavigationEnabled())
            return nullptr;
        const StyleNavData* nav = style.nav(propertyID);
        if (!nav || !nav->flag)
            return CSSPrimitiveValue::createIdentifier(CSSValueAuto);

        const CSSValue* valId = CSSCustomIdentValue::create(nav->id);
        if (nav->flag == StyleNavData::NAV_TARGET_CURRENT)
            return valId;

        CSSValueList* list = CSSValueList::createSpaceSeparated();
        list->append(*valId);
        if (nav->flag == StyleNavData::NAV_TARGET_NAME)
            list->append(*CSSCustomIdentValue::create(nav->target));
        else
            list->append(*CSSPrimitiveValue::createIdentifier(CSSValueRoot));
