...
const HeapVector<Member<CSSStyleSheet>>& StyleEngine::injectedAuthorStyleSheets() const
{
...
void StyleEngine::updateInjectedStyleSheetCache() const
{
    ...
    for (unsigned i = 0; i < entries.size(); ++i) {
        ...
        if (entry->injectedFrames() == InjectStyleInTopFrameOnly && document().localOwner())
            continue;
        if (!URLPatternMatcher::matchesPatterns(document().url(), entry->whitelist()))
            continue;
        CSSStyleSheet* groupSheet = CSSStyleSheet::createInline(m_document, KURL());
