...
MediaStateByAppObserver::~MediaStateByAppObserver()
{
}
...
Document::~Document()
{
    ...
    ASSERT(m_mediaStateObservers.isEmpty());
...
bool Document::accessibilityEnabled() const
{
#if OS(WEBOS)
    if (base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kWebOSWAM))
        return settings() && settings()->audioGuidanceOn();
#endif
    return false;
}

bool Document::accessibilityReady() const
{
#if OS(WEBOS)
    if (base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kWebOSWAM))
        return settings() && settings()->accessibilityEnabled();
#endif
    return false;
}
...
void Document::suspendMediaByApp()
{
    m_appSuspended = true;
    MediaStateByAppObserverSet::const_iterator observerEnd = m_mediaStateObservers.end();
    for (MediaStateByAppObserverSet::const_iterator it =  m_mediaStateObservers.begin(); it != observerEnd; ++it)
        (*it)->suspendMediaByApp();
}

void Document::resumeMediaByApp()
{
    m_appSuspended = false;
    MediaStateByAppObserverSet::const_iterator observerEnd = m_mediaStateObservers.end();
    for (MediaStateByAppObserverSet::const_iterator it =  m_mediaStateObservers.begin(); it != observerEnd; ++it)
...
void Document::contentsPositionChanged(float position)
{
    MediaStateByAppObserverSet::const_iterator observerEnd = m_mediaStateObservers.end();
    for (MediaStateByAppObserverSet::const_iterator it = m_mediaStateObservers.begin(); it != observerEnd; ++it)
        (*it)->contentsPositionChanged(position);
}

void Document::registerMediaStateObserver(MediaStateByAppObserver* observer)
{
    ASSERT(!m_mediaStateObservers.contains(observer));
    m_mediaStateObservers.add(observer);
}

void Document::unregisterMediaStateObserver(MediaStateByAppObserver* observer)
{
    ASSERT(m_mediaStateObservers.contains(observer));
    m_mediaStateObservers.remove(observer);
}
...
int Document::updateImages(int scrollOffset, std::vector<HTMLImageElement*>& imageElements)
{
    if (!documentElement() || loadEventFinished() == false)
        return 0;
    ...
    FrameView* frameView = view();
    if (frameView)
        frameView->updateAbsolutePosition();

    int count = 0;
    for (HTMLElement* child = Traversal<HTMLElement>::firstChild(*body); child; child = Traversal<HTMLElement>::nextSibling(*child))
        count += child->updateImages(scrollOffset, imageElements);
    return count;
}
...
bool Document::setFocusedElement(Element* prpNewFocusedElement, const FocusParams& params)
{
    ...
    if (!focusChangeBlocked && m_focusedElement && (params.type != WebFocusTypeMouse)) {
...
void Document::beginLifecycleUpdatesIfRenderingReady()
{
    ...
    if (LocalFrame* frame = this->frame()) {
        ...
        if (m_deferredBackgroundImageCount > 0)
            return;
#endif
...
void Document::initSecurityContext(const DocumentInit& initializer)
{
    ...
    if (Settings* settings = initializer.settings()) {
        ...
        if (settings->allowLocalResourceLoad())
            getSecurityOrigin()->grantLoadLocalResources();
    }
...
{
    if (!isMainThread() || !m_frame.get() || !m_frame->isMainFrame())
        return false;

    // pause update when the first background image was deferred
    if (m_deferredBackgroundImageCount == 0)
        m_frame->page()->chromeClient().pauseLifecycleUpdates();

    ++m_deferredBackgroundImageCount;
    return true;
}
...
void Document::removeDeferredBackgroundImage()
{
    ...
    if (m_deferredBackgroundImageCount == 0)
        m_frame->page()->chromeClient().beginLifecycleUpdates();
}
#endif
...
DEFINE_TRACE(Document)
{
    ...
    visitor->trace(m_mediaStateObservers);
