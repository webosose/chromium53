...
class CORE_EXPORT MediaStateByAppObserver : public GarbageCollectedMixin {
public:
    virtual ~MediaStateByAppObserver();

    virtual void suspendMediaByApp() = 0;
    virtual void resumeMediaByApp() = 0;
...
public:
    ...
    */
    ...
    int updateImages(int scrollOffset, std::vector<HTMLImageElement*>& images);
    ...
    bool appSuspended() const { return m_appSuspended; }
    void suspendMediaByApp();
    void resumeMediaByApp();
    void contentsPositionChanged(float position);

    void registerMediaStateObserver(MediaStateByAppObserver*);
    void unregisterMediaStateObserver(MediaStateByAppObserver*);
...
private:
    ...
    typedef HeapHashSet<WeakMember<MediaStateByAppObserver>> MediaStateByAppObserverSet;
    MediaStateByAppObserverSet m_mediaStateObservers;
    bool m_appSuspended;
...
#if defined(OS_WEBOS)
    int m_deferredBackgroundImageCount = 0;
#endif
