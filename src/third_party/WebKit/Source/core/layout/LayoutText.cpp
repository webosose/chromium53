...
{
    //If previos text is empty, and current text is 1 character then we have to again set text
    Node* e = node();
    if (e && e->isEditingText())
        if (m_previousText.length() == 0 && text().impl()->length() == 1)
            setText(text().impl(), true);
}
