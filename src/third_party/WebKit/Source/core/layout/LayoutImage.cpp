...
void LayoutImage::imageNotifyFinished(ImageResource* newImage)
{
    ...
    if (blink::Platform::current()->useDynamicRefetch() && imageElt)
        imageElt->notifyFinished();
...
int LayoutImage::imagePosition(int scrollPosition)
{
    if (view() && view()->frameView()) {
        int framePosition = view()->frameView()->getAbsoutePosition();
        framePosition -= scrollPosition;
        framePosition -= view()->frameView()->scrollOffset().height();
        return (int)localToAbsolute().y() + framePosition;
    }
    return -1;
}

const bool LayoutImage::isVisible() const
{
