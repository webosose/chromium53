...
LayoutUnit InlineTextBox::placeEllipsisBox(bool flowIsLTR, LayoutUnit visibleLeftEdge, LayoutUnit visibleRightEdge, LayoutUnit ellipsisWidth, LayoutUnit &truncatedWidth, bool& foundBox)
{
    ...
    if (ltrEllipsisWithinBox || rtlEllipsisWithinBox) {
        ...
        if (offset == 0) {
            ...
            // FIXME: return -1 is workaround code. We need to check offsetForPosition and logicalLeft() value.
            // It is relate with bidi bracket automation algorithm.
            return flowIsLTR ? std::min(ellipsisX, logicalLeft()) : LayoutUnit(-1);
        }
