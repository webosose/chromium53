...
void Resource::CachedMetadataHandlerImpl::setSerializedCachedMetadata(const char* data, size_t size)
{
    ...
    PMLOG_DEBUG("V8CodeCache", "V8CodeCache Consume %s(%d)", m_resource->url().getString().utf8().data(), size);
}
...
void Resource::CachedMetadataHandlerImpl::sendToPlatform()
{
    if (m_cachedMetadata) {
        ...
        PMLOG_DEBUG("V8CodeCache", "V8CodeCache Produce %s(%d)", m_resource->url().getString().utf8().data(), serializedData.size());
    } else {
...
void Resource::setLoader(ResourceLoader* loader)
{
    ...
    if (loader->fetcher()->localResourceCodeCacheEnabled() && m_type == Resource::Script && m_resourceRequest.url().isLocalFile()) {
        // Avoid code caching from file URIs with query string
        if (!loader->fetcher()->codeCacheFromFileURIsWithQueryStringDisallowed() || m_resourceRequest.url().query().isNull())
            m_cacheHandler = CachedMetadataHandlerImpl::create(this);
    }
}
