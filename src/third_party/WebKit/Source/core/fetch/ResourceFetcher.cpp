...
ResourceFetcher::ResourceFetcher(FetchContext* newContext)
    ...
    , m_localResourceCodeCacheEnabled(false)
    , m_codeCacheFromFileURIsWithQueryStringDisallowed(false)
{
...
ResourceFetcher::RevalidationPolicy ResourceFetcher::determineRevalidationPolicy(Resource::Type type, const FetchRequest& fetchRequest, Resource* existingResource, bool isStaticData) const
{
    ...
    if (blink::Platform::current()->useDynamicRefetch() && existingResource->url().getString().find(blink::Platform::current()->getDynamicRefetchDummyFile()) != std::string::npos)
        return Use;
...
{
    if (enable == m_localResourceCodeCacheEnabled)
        return;

    m_localResourceCodeCacheEnabled = enable;
}

void ResourceFetcher::setCodeCacheFromFileURIsWithQueryStringDisallowed(bool enable)
{
    if (enable == m_codeCacheFromFileURIsWithQueryStringDisallowed)
        return;

     m_codeCacheFromFileURIsWithQueryStringDisallowed = enable;
}
