...
public:
    ...
    void setLoad(bool flag) { m_loaded = flag; }
    virtual int updateImages(int scrollOffset, std::vector<HTMLImageElement*>& imageElements) override;

    const int getOriginalWidth() const { return m_original_width; }
    const int getOriginalHeight() const { return m_original_height; }
    void notifyFinished();

    String original() { return m_original; }
    String fileToLoad() { return m_fileToLoad; }

    bool firstLoaded() { return m_firstLoaded; }

    int position() { return m_position; }
    int heightO() { return m_height; }

    void setPositionAndHeight(int position, int height)
    {
        m_position = position;
        m_height = height;
    }
