...
int HTMLFrameElementBase::updateImages(int scrollOffset, std::vector<HTMLImageElement*>& images)
{
    if (document().isHTMLDocument() && !isInShadowTree()) {
        Document* contentDoc = contentDocument();
        if (contentDoc)
            return contentDoc->updateImages(scrollOffset, images);
    }
    return 0;
}
