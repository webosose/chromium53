...
namespace blink {
static bool g_cancel_refetch = false;
...
void HTMLImageElement::attach(const AttachContext& context)
{
    ...
    if (layoutObject() && layoutObject()->isImage()) {
        ...
        if (blink::Platform::current()->useDynamicRefetch() && imageSourceURL() == blink::Platform::current()->getDynamicRefetchDummy())
            g_cancel_refetch = true;
...
void HTMLImageElement::selectSourceURL(ImageLoader::UpdateFromElementBehavior behavior)
{
    ...
    if (blink::Platform::current()->useDynamicRefetch() && m_loaded == false)
        imageLoader().updateFromElement(ImageLoader::UpdateIgnorePreviousError, m_referrerPolicy);
    else
...
int HTMLImageElement::unload()
{
    if (g_cancel_refetch == true || m_loaded == false || m_firstLoaded == false)
        return 0;

    if (m_original_width == 0 && m_original_height == 0 && imageLoader().image()) {
        IntSize imageSize = bitmapSourceSize();
        m_original_width = imageSize.width();
        m_original_height = imageSize.height();
    }

    if (m_original_size == 0 && cachedImage())
        m_original_size = cachedImage()->encodedSize();

    if (getOriginalWidth() * getOriginalHeight() < 200 * 200 || m_original_size < 20 * 1024)
        return 0;

    if (imageLoader().hasPendingActivity())
        return 0;

    m_original = imageSourceURL();
    m_fileToLoad = blink::Platform::current()->getDynamicRefetchDummy();
    m_fileToLoad.append("#");
    m_fileToLoad.append(m_original);
...
int HTMLImageElement::load()
{
    if (m_loaded == true)
        return 0;
    if (imageSourceURL().find(blink::Platform::current()->getDynamicRefetchDummyFile()) != std::string::npos)
        m_fileToLoad = m_original;
    else
        m_original = m_fileToLoad = imageSourceURL();
...
void HTMLImageElement::notifyFinished()
{
    m_firstLoaded = true;
}

int HTMLImageElement::updateImages(int scrollOffset, std::vector<HTMLImageElement*>& imageElements)
{
    if (layoutObject() && layoutObject()->isImage()) {
        LayoutImage* ri = toLayoutImage(layoutObject());
        m_position = ri->imagePosition(scrollOffset);

        if (m_original_height)
            m_height = m_original_height;
        else
            m_height = bitmapSourceSize().height();

        imageElements.push_back(this);
    }
    return 0;
}
