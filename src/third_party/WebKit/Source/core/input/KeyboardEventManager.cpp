    ...
    const PlatformKeyboardEvent& initialKeyEvent)
{
    ...
    if (initialKeyEvent.type() == PlatformEvent::KeyUp || initialKeyEvent.type() == PlatformEvent::Char) {
        ...
        domEvent = KeyboardEvent::create(initialKeyEvent, m_frame->document()->domWindow());
    ...
    keydown = KeyboardEvent::create(keyDownEvent, m_frame->document()->domWindow());
    if (matchedAnAccessKey)
        keydown->setDefaultPrevented(true);
    keydown->setTarget(node);

#if defined(OS_WEBOS)
    if (keydown->key() == "RCUBack" && m_frame->settings()->webOSBackHistoryAPI()) {
        // By default webOSBackHistoryAPI:false in Settings.h
        // But webapp default back key handling(webOSBackHistoryAPI:true): App doesn't receive back key event
        defaultRCUBackEventHandler(keydown);
...
void KeyboardEventManager::defaultArrowEventHandler(WebFocusType focusType, KeyboardEvent* event)
{
    ...
    if (!RuntimeEnabledFeatures::cssNavigationEnabled() && !isSpatialNavigationEnabled(m_frame))
        return;
...
#if defined(OS_WEBOS)
void KeyboardEventManager::defaultRCUBackEventHandler(KeyboardEvent* event)
{
    DCHECK_EQ(event->type(), EventTypeNames::keydown);
    UseCounter::count(m_frame->document(), UseCounter::BackspaceNavigatedBack);
    if (m_frame->page()->chromeClient().hadFormInteraction())
