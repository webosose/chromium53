...
static const int dynamicRefetchVisibilityUpdate = 1000;
static const int dynamicRefetchVisibleArea = 4000;
static const int dynamicRefetchMaxReplace = 300;
...
FrameView::FrameView(LocalFrame* frame)
    ...
    , m_lastUnloadPosition(0)
    , m_unloadCount(0)
    , m_unloadTimer(this, &FrameView::didUnloadTimerFired)
    , m_scrollTo(false)
    , m_framePosition(0)
{
    ASSERT(m_frame);
...
FrameView::~FrameView()
{
    if (blink::Platform::current()->useDynamicRefetch() && m_unloadTimer.isActive())
        m_unloadTimer.stop();
...
void FrameView::init()
{
    ...
    if (m_frame->isMainFrame() && m_frame->settings()->disallowScrollbarsInMainFrame())
        setCanHaveScrollbars(false);

    if (m_frame->isMainFrame() && blink::Platform::current()->useDynamicRefetch())
        m_unloadTimer.startOneShot(1.0, BLINK_FROM_HERE);
}
...
void FrameView::onlyVisibleImageInMemory()
{
    Document* document = m_frame->document();
    if (document == NULL)
        return;

    int scrollO = scrollOffset().height();
    if (m_frame->isMainFrame() == false && m_frame->page() && m_frame->page()->mainFrame())
        scrollO = toLocalFrame(m_frame->page()->mainFrame())->view()->scrollOffset().height();
    if (m_frame->isMainFrame())
        scrollO = 0;

    std::vector<HTMLImageElement*> imageElements;
    imageElements.reserve(dynamicRefetchMaxReplace);
    document->updateImages(scrollO, imageElements);
    std::sort(imageElements.begin(), imageElements.end(),
        [](HTMLImageElement* i, HTMLImageElement* j) { return (i->position() > j->position()); });

    int count = 0;
    for (auto it = imageElements.begin(); it != imageElements.end(); ++it) {
        int position = (*it)->position();
        int height = (*it)->heightO();

        if (count < dynamicRefetchMaxReplace && ((position > dynamicRefetchVisibleArea) || (position + height < -dynamicRefetchVisibleArea)))
            count += ((*it)->unload()) ? 1 : 0;
        else
            (*it)->load();
    }
...
void FrameView::handleLoadCompleted()
{
    ...
    if (blink::Platform::current()->useDynamicRefetch() && m_frame && m_frame->isMainFrame()
        && m_lastUnloadPosition < dynamicRefetchVisibilityUpdate)
        onlyVisibleImageInMemory();
}
...
void FrameView::didUnloadTimerFired(Timer<FrameView>*)
{
    m_unloadTimer.stop();
    m_unloadCount++;

    if (m_unloadCount < 20)
        m_unloadTimer.startOneShot(2, BLINK_FROM_HERE);
...
void FrameView::setScrollOffset(const DoublePoint& offset, ScrollType scrollType)
{
    ...
    if (blink::Platform::current()->useDynamicRefetch() && m_frame && abs(m_lastUnloadPosition - scrollOffset().height()) > dynamicRefetchVisibilityUpdate) {
        m_lastUnloadPosition = scrollOffset().height();
        m_scrollTo = true;
        onlyVisibleImageInMemory();
        m_scrollTo = false;
    }
...
void FrameView::updateAbsolutePosition()
{
    if (!blink::Platform::current()->useDynamicRefetch())
        return;

    if (m_frame->isMainFrame() == false)
        m_framePosition = convertToRootFrame(IntRect(0, 0, 0, 0)).y();
    else
        m_framePosition = y();
}
