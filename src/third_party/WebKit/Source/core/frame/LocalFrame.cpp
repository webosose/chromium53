...
void LocalFrame::suspendMediaByApp()
{
    if (document())
        document()->suspendMediaByApp();
    ...
    for (size_t i = 0; i < childFrames.size(); ++i)
        childFrames[i]->suspendMediaByApp();
}

void LocalFrame::resumeMediaByApp()
{
    if (document())
        document()->resumeMediaByApp();
    ...
    for (size_t i = 0; i < childFrames.size(); ++i)
        childFrames[i]->resumeMediaByApp();
}

void LocalFrame::contentsPositionChanged(float position) {
    if (document())
        document()->contentsPositionChanged(position);
    ...
    for (size_t i = 0 ; i < childFrames.size(); ++i)
        childFrames[i]->contentsPositionChanged(position);
}
