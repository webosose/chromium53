...
void Page::suspendActiveDOMObjects()
{
#if OS(WEBOS)
    for (Frame* frame = mainFrame(); frame; frame = frame->tree().traverseNextWithWrap(false)) {
        if (frame->isLocalFrame() && toLocalFrame(frame)->document())
            toLocalFrame(frame)->document()->suspendActiveDOMObjects();
    }
#endif
}

void Page::resumeActiveDOMObjects()
{
#if OS(WEBOS)
    for (Frame* frame = mainFrame(); frame; frame = frame->tree().traverseNextWithWrap(false)) {
        if (frame->isLocalFrame() && toLocalFrame(frame)->document())
            toLocalFrame(frame)->document()->resumeActiveDOMObjects();
    }
#endif
}
...
void Page::suspendMediaByApp()
{
    if (mainFrame() && mainFrame()->isLocalFrame())
        deprecatedLocalMainFrame()->suspendMediaByApp();
}

void Page::resumeMediaByApp()
{
    if (mainFrame() && mainFrame()->isLocalFrame())
        deprecatedLocalMainFrame()->resumeMediaByApp();
}
...
void Page::settingsChanged(SettingsDelegate::ChangeType changeType)
{
    ...
    case SettingsDelegate::TextTrackKindUserPreferenceChange:
        ...
        break;
    case SettingsDelegate::LocalResourceLoadChange:
        if (settings().allowLocalResourceLoad()) {
            if (mainFrame() && mainFrame()->isLocalFrame())
                deprecatedLocalMainFrame()->document()->getSecurityOrigin()->grantLoadLocalResources();
        }
        break;
    }
}
