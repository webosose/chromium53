...
{
    if (m_scrollableArea->isFrameView()) {
        FrameView* frame_view = static_cast<FrameView*>(m_scrollableArea.get());

        if (frame_view->frame().settings() && frame_view->frame().settings()->webOSNativeScrollEnabled())
            return true;
        return false;
    }

    if (m_scrollableArea->isPaintLayerScrollableArea()) {
        PaintLayerScrollableArea* plsa = static_cast<PaintLayerScrollableArea*>(m_scrollableArea.get());

        if (plsa->box().document().settings() && plsa->box().document().settings()->webOSNativeScrollEnabled())
            return true;
        return false;
    }
...
void ProgrammaticScrollAnimator::animateToOffset(FloatPoint offset)
{
    ...
    if (isWebOSNativeScrollEnabled()) {
        ...
        bool isRenewAnimation = true;
        if (m_runState == RunState::Idle || m_runState == RunState::PostAnimationCleanup)
            isRenewAnimation = false;
        ...
        m_animationCurve = CompositorScrollOffsetAnimationCurve::create(
            ...
            isRenewAnimation ? CompositorScrollOffsetAnimationCurve::EASE_OUT : CompositorScrollOffsetAnimationCurve::EASE_IN_OUT);
    } else {
        m_animationCurve = CompositorScrollOffsetAnimationCurve::create(
            compositorOffsetFromBlinkOffset(m_targetOffset),
            CompositorScrollOffsetAnimationCurve::ScrollDurationDeltaBased,
