...
void V8Initializer::Initialize(IsolateHolder::ScriptMode mode,
  ...
  if (base::CommandLine::ForCurrentProcess()->HasSwitch(
    ...
    base::FilePath file_path =
        base::CommandLine::ForCurrentProcess()->GetSwitchValuePath(
            switches::kV8SnapshotBlobPath);
    base::MemoryMappedFile* custom_snapshot = new base::MemoryMappedFile;
    if (custom_snapshot->Initialize(file_path)) {
      mapped_snapshot = custom_snapshot;
      RAW_PMLOG_DEBUG("V8 Custom snapshot_blob loaded: %s",
  ...
  if (mapped_snapshot != NULL) {
    ...
    snapshot.raw_size = static_cast<int>(mapped_snapshot->length());
