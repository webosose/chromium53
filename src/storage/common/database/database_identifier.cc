...
#if defined(OS_WEBOS)
  std::string databaseIdentifer = origin.database_identifier();
  if(!databaseIdentifer.empty()) {
    std::string removedString = "file://";
    size_t pos;

    while( (pos = databaseIdentifer.find(removedString)) != std::string::npos)
        databaseIdentifer.erase(pos,removedString.length());

    removedString.erase(0, strlen("file:/"));  //  remove '/'

    while( (pos = databaseIdentifer.find(removedString)) != std::string::npos)
        databaseIdentifer.erase(pos,removedString.length());

    return DatabaseIdentifier(origin.scheme(), databaseIdentifer, 0, false, false);
  }
#endif
...
#if defined(OS_WEBOS)
  if (scheme != "file" && (!url.is_valid() || url.scheme() != scheme || url.host() != hostname))
#else
