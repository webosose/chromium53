...
struct PP_EncryptedBlockInfo {
  ...
  /**
   * Indicates if the block contains video data
   */
  uint32_t is_video;

  /**
   * Extra field added to make the struct size exactly 376 bytes
   * so that the qemux86 build will pass. On qemux86 the structs
   * are packed to 4 bytes, for the arm build structs are packed
   * to 8 bytes. The extra 'padding' field ensures that the struct
   * size is exactly 376 bytes for both these builds
   */
  uint32_t padding;
};
PP_COMPILE_ASSERT_STRUCT_SIZE_IN_BYTES(PP_EncryptedBlockInfo, 376);
/**
 * @}
 */
