...
class CONTENT_EXPORT WebContentsDelegate {
 public:
  ...
  // Notifies the delegate that history.back() is called but there is nowhere to go back
  ...
  // Check the it has policy for response, if it has a policy, Blink won't do nothing   491
  virtual bool DecidePolicyForResponse(
      bool isMainFrame, int statusCode,
      const GURL& url,
      const base::string16& statusText) { return false; }
   // Notifies the delegate that renderer frame Focused for the first time
  virtual void DidFirstFrameFocused() {}

  // Notifies the delegate that render did first non blank paint
  virtual void DidFirstNonBlankPaint() {}
