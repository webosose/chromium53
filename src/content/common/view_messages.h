...
IPC_STRUCT_TRAITS_BEGIN(blink::WebScreenInfo)
  IPC_STRUCT_TRAITS_MEMBER(additionalContentsScale)
  IPC_STRUCT_TRAITS_MEMBER(deviceScaleFactor)
...
IPC_MESSAGE_ROUTED0(ViewMsg_ResumeMedia)
IPC_MESSAGE_ROUTED0(ViewMsg_SuspendDOM)
IPC_MESSAGE_ROUTED0(ViewMsg_ResumeDOM)
IPC_MESSAGE_ROUTED1(ViewMsg_ContentsPositionChanged,
                    float /* blocked */)
...
IPC_MESSAGE_ROUTED1(ViewMsg_ReplaceBaseURL,
                    GURL)
...
// Notifies the browser that renderer frame Focused for the first time
IPC_MESSAGE_ROUTED0(ViewHostMsg_DidFirstFrameFocused)
...
// Notifies the browser that renderer painted non-blank page
IPC_MESSAGE_ROUTED0(ViewHostMsg_DidFirstNonBlankPaint)
#endif

#if defined(OS_WEBOS)
// Notifies the renderer process that system UI language has changed
IPC_MESSAGE_CONTROL1(ViewMsg_NotifyLocaleChanged, std::string /* locale */)
#endif
