...
#if defined(OS_WEBOS)
const int kModerateMemoryPressureDivider = 4;
const size_t kMinimalLimitBytes = 8 * 1024 * 1024;
#endif
...
int64_t GetDefaultMemoryLimit() {
  ...
  // matches WebOS chromium44 GpuMemoryManager limits, however in chromium53
  ...
  if (webos_system_memory_mb >= 1152)
    webos_system_memory_mb = webos_system_memory_mb / 8;
  else if (webos_system_memory_mb >= 768)
    webos_system_memory_mb = webos_system_memory_mb / 10;
  else
    webos_system_memory_mb = webos_system_memory_mb / 12;
...
void HostDiscardableSharedMemoryManager::OnMemoryPressure(
  ...
  switch (memory_pressure_level) {
    ...
    case base::MemoryPressureListener::MEMORY_PRESSURE_LEVEL_MODERATE:
      ReduceMemoryUsageUntilWithinLimit(std::max(
          memory_limit_ / kModerateMemoryPressureDivider, kMinimalLimitBytes));
      break;
    case base::MemoryPressureListener::MEMORY_PRESSURE_LEVEL_CRITICAL:
      ReduceMemoryUsageUntilWithinLimit(kMinimalLimitBytes);
      break;
  }
#else
  switch (memory_pressure_level) {
