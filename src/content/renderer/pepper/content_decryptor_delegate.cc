...
bool ContentDecryptorDelegate::Decrypt(
  ...
  uint32_t is_video = (stream_type == Decryptor::StreamType::kVideo) ? 1 : 0;
  if (!MakeEncryptedBlockInfo(encrypted_buffer, request_id, is_video, &block_info)) {
    return false;
  }
...
bool ContentDecryptorDelegate::DecryptAndDecodeAudio(
  ...
  PP_EncryptedBlockInfo block_info = {};
  if (!MakeEncryptedBlockInfo(encrypted_buffer, request_id, 0, &block_info)) {
    return false;
  }
...
bool ContentDecryptorDelegate::DecryptAndDecodeVideo(
  ...
  PP_EncryptedBlockInfo block_info = {};
  if (!MakeEncryptedBlockInfo(encrypted_buffer, request_id, 1, &block_info)) {
    return false;
  }
