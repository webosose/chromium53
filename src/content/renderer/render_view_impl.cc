...
void RenderViewImpl::Initialize(const ViewMsg_New_Params& params,
  ...
  webview()->settings()->setLocalResourceCodeCacheEnabled(
      command_line.HasSwitch(switches::kEnableLocalResourceCodeCache));
  webview()->settings()->setCodeCacheFromFileURIsWithQueryStringDisallowed(
      command_line.HasSwitch(switches::kDisallowCodeCacheFromFileURIsWithQueryString));
  ...
  if (!network_stable_timeout.empty()) {
    ...
    webview()->settings()->setNetworkStableTimeout(network_stable_seconds);
  }
#endif
  ...
  webview()->settings()->setWebOSNativeScrollEnabled(
...
void RenderView::ApplyWebPreferences(const WebPreferences& prefs,
  ...
  settings->setAccessibilityExploreByMouseEnabled(
      settings->accessibilityExploreByMouseEnabled() &&
  ...
  settings->setForceVideoTexture(prefs.force_video_texture);
  ...
  // back_history_api_disabled is changed to webOSBackHistoryAPI
  settings->setWebOSBackHistoryAPI(!prefs.back_history_api_disabled);
  ...
  settings->setMouseOnOffEventEnabled(prefs.allow_mouse_on_off_event);
  WebRuntimeFeatures::enableMouseOnOffEvent(prefs.allow_mouse_on_off_event);
}
...
bool RenderViewImpl::OnMessageReceived(const IPC::Message& message) {
  ...
  IPC_BEGIN_MESSAGE_MAP(RenderViewImpl, message)
    ...
    IPC_MESSAGE_HANDLER(ViewMsg_SetUseLaunchOptimization, OnSetUseLaunchOptimization)
    IPC_MESSAGE_HANDLER(ViewMsg_SetBlockWriteDiskcache,
                        OnSetBlockWriteDiskcache)
    IPC_MESSAGE_HANDLER(ViewMsg_SuspendMedia, OnSuspendMedia)
    IPC_MESSAGE_HANDLER(ViewMsg_ResumeMedia, OnResumeMedia)
    IPC_MESSAGE_HANDLER(ViewMsg_SuspendDOM, OnSuspendDOM)
    IPC_MESSAGE_HANDLER(ViewMsg_ResumeDOM, OnResumeDOM)
    IPC_MESSAGE_HANDLER(ViewMsg_ContentsPositionChanged, OnContentsPositionChanged)
    IPC_MESSAGE_HANDLER(ViewMsg_ResetStateToMarkNextPaintForContainer, OnResetStateToMarkNextPaintForContainer)
...
void RenderViewImpl::SetPreferredLanguages(const std::string& languages) {
  ...
  while (t.GetNext())
    preferred_languages.push_back(t.token());
...
void RenderViewImpl::OnSetBlockWriteDiskcache(bool blocked) {
  block_write_diskcache_ = blocked;
}
...
void RenderViewImpl::OnResetStateToMarkNextPaintForContainer() {
  if (webview()->mainFrame()->isWebLocalFrame())
    webview()->mainFrame()->resetStateToMarkNextPaintForContainer();
  if (main_render_frame_)
    main_render_frame_->ResetStateToMarkNextPaintForContainer();
}
...
void RenderViewImpl::SetKeepAliveWebApp(bool keepAlive) {
  if (webview())
    if (webview()->settings())
      webview()->settings()->setKeepAliveWebApp(keepAlive);
}
#endif
...
void RenderViewImpl::didFirstFrameFocused() {
  Send(new ViewHostMsg_DidFirstFrameFocused(routing_id_));
}
...
void RenderViewImpl::OnSetRendererPrefs(
  ...
  if(!media_codec_capability.empty())
    media::MediaAPIsWrapper::CheckCodecInfo(media_codec_capability);
#endif
...
void RenderViewImpl::OnPageWasHidden() {
  ...
  const base::CommandLine& command_line =
      *base::CommandLine::ForCurrentProcess();
  if (command_line.HasSwitch(switches::kWebRTCDisconnectPeersOnPageHidden))
    DropAllPeerConnections(DROP_PEER_CONNECTION_REASON_PAGE_HIDDEN);
#endif

#if (defined(OS_ANDROID) || defined(OS_WEBOS)) && defined(ENABLE_WEBRTC)
...
#if defined(OS_WEBOS) && defined(ENABLE_WEBRTC)
  RenderThreadImpl::current()->audio_input_message_filter()->SuspendDevices(
      true);
#endif
...
void RenderViewImpl::OnResumeMedia() {
  if (!webview())
    return;
...
void RenderViewImpl::OnSuspendDOM() {
  if (!webview())
    return;
  if (!dom_suspended_) {
    ...
    RAW_PMLOG_INFO("[SuspendDOM]", "Already suspended;return");
}

void RenderViewImpl::OnResumeDOM() {
  if (!webview())
    return;
  if (dom_suspended_) {
    ...
    RAW_PMLOG_INFO("[ResumeDOM]", "Not suspended;Nothing to resume;return");

}

void RenderViewImpl::OnContentsPositionChanged(float position) {
  if (webview())
    webview()->contentsPositionChanged(position);
}
...
blink::WebString RenderViewImpl::applicationId() {
  return WebString::fromUTF8(renderer_preferences_.application_id);
}

blink::WebString RenderViewImpl::boardType() {
  return WebString::fromUTF8(renderer_preferences_.board_type);
}

blink::WebString RenderViewImpl::mediaCodecCapability() {
  return WebString::fromUTF8(renderer_preferences_.media_codec_capability);
}
}  // namespace content
