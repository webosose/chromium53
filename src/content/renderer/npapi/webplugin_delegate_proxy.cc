...
bool WebPluginDelegateProxy::Initialize(
  ...
  for (size_t attempt = 0; !result && attempt < kAttempts; attempt++) {
    ...
    std::unique_ptr<base::AutoReset<bool> > track_nested_removes(
...
void WebPluginDelegateProxy::DidReceiveManualResponse(
    ...
    const std::string& headers, uint32_t expected_length,
    uint32_t last_modified) {
  PluginMsg_DidReceiveResponseParams params;
  params.id = 0;
  params.mime_type = mime_type;
...
bool WebPluginDelegateProxy::OnMessageReceived(const IPC::Message& msg) {
  ...
  IPC_BEGIN_MESSAGE_MAP(WebPluginDelegateProxy, msg)
    ...
    IPC_MESSAGE_HANDLER(PluginHostMsg_GetUserAgent, OnGetUserAgent)
...
#endif
#if defined(OS_WEBOS)
    IPC_MESSAGE_HANDLER(PluginHostMsg_PluginEnabledRendering,
                        OnPluginEnabledRendering)
    IPC_MESSAGE_HANDLER(PluginHostMsg_AcceleratedPluginEnabledRendering,
                        OnAcceleratedPluginEnabledRendering)
    IPC_MESSAGE_HANDLER(PluginHostMsg_AcceleratedPluginAllocateBuffer,
                        OnAcceleratedPluginAllocateBuffer)
    IPC_MESSAGE_HANDLER(PluginHostMsg_AcceleratedPluginReleaseBuffer,
                        OnAcceleratedPluginReleaseBuffer)
    IPC_MESSAGE_HANDLER(PluginHostMsg_AcceleratedPluginSwapBuffer,
                        OnAcceleratedPluginSwapBuffer)
    IPC_MESSAGE_HANDLER(PluginHostMsg_SetUsesPunchHoleForVideo,
                        OnSetUsesPunchHoleForVideo)
#endif
...
bool WebPluginDelegateProxy::CreateLocalBitmap(
  ...
  if (memory->size() != size)
    return false;
  *canvas = std::unique_ptr<SkCanvas>(skia::CreatePlatformCanvas(
...
bool WebPluginDelegateProxy::CreateSharedBitmap(
    std::unique_ptr<SharedMemoryBitmap>* memory,
    std::unique_ptr<SkCanvas>* canvas) {
...
void WebPluginDelegateProxy::Paint(SkCanvas* canvas,
...
#if defined(PUNCH_HOLE_FOR_PLUGINS)
#if defined(OS_WEBOS)
  if (plugin_->GetUsesPunchHoleForVideo()) {
#endif
    SkPaint clear_paint;
  ...
  SkPaint paint;
  paint.setXfermodeMode(transparent_ ? SkXfermode::kSrcOver_Mode
...
#if defined(OS_WEBOS)
void WebPluginDelegateProxy::Suspend() {
  Send(new PluginMsg_Suspend(instance_id_));
}

void WebPluginDelegateProxy::Resume() {
  Send(new PluginMsg_Resume(instance_id_));
}

void WebPluginDelegateProxy::SetDeviceInfo(
    const gfx::Size& device_viewport_size,
    const gfx::Size& view_size) {
  Send(new PluginMsg_SetDeviceInfo(instance_id_, device_viewport_size,
                                   view_size));
}
#endif
...
void WebPluginDelegateProxy::OnGetUserAgent(std::string *user_agent) {
  if (render_frame_)
    *user_agent = render_frame_->GetWebContentUserAgent();
  else
    *user_agent = content::GetContentClient()->GetUserAgent();
}

#if defined(OS_WEBOS)
void WebPluginDelegateProxy::OnPluginEnabledRendering() {
  uses_shared_bitmaps_ = true;
}
...
void WebPluginDelegateProxy::OnAcceleratedPluginEnabledRendering(
  ...
  uses_shared_bitmaps_ = false;
  if (plugin_)
    plugin_->AcceleratedPluginEnabledRendering(paintless);
}

void WebPluginDelegateProxy::OnAcceleratedPluginAllocateBuffer(
    uint32_t width,
    uint32_t height,
  ...
  if (plugin_)
    plugin_->AcceleratedPluginAllocateBuffer(width, height, internalformat,
                                             usage, format, id, handle);
}

void WebPluginDelegateProxy::OnAcceleratedPluginReleaseBuffer(int32_t id) {
  if (plugin_)
    plugin_->AcceleratedPluginReleaseBuffer(id);
}

void WebPluginDelegateProxy::OnAcceleratedPluginSwapBuffer(int32_t id) {
  if (plugin_)
    plugin_->AcceleratedPluginSwapBuffer(id);
}

void WebPluginDelegateProxy::AcceleratedPluginDidSwapBuffer(int32_t id) {
  Send(new PluginMsg_AcceleratedPluginDidSwapBuffer(instance_id_, id));
}

void WebPluginDelegateProxy::AcceleratedPluginDidSwapBufferComplete(
    int32_t id) {
  Send(new PluginMsg_AcceleratedPluginDidSwapBufferComplete(instance_id_, id));
}

void WebPluginDelegateProxy::OnSetUsesPunchHoleForVideo(
    bool uses_punch_hole_for_video) {
  if (plugin_)
    plugin_->SetUsesPunchHoleForVideo(uses_punch_hole_for_video);
}
#endif
