...
#if defined(OS_WEBOS)
    delegate_->SetDeviceInfo(render_view_->physical_backing_size(),
                             render_view_->webview()->size());
#endif
  }
...
#if defined(OS_WEBOS)
void WebPluginImpl::suspend() {
  if (!delegate_)
    return;
  delegate_->Suspend();
}

void WebPluginImpl::resume() {
  if (!delegate_)
    return;
  delegate_->Resume();
}
...
WebPluginImpl::WebPluginImpl(WebFrame* webframe,
    ...
    : render_frame_(render_frame),
      ...
      mime_type_(base::ToLowerASCII(
          base::UTF16ToASCII(base::StringPiece16(params.mimeType)))),
      loader_client_(this),
      weak_factory_(this) {
  uses_punch_hole_for_video_ = (mime_type_ != kFlashPluginSwfMimeType);
...
bool WebPluginImpl::GetUsesPunchHoleForVideo() {
  return uses_punch_hole_for_video_;
}
...
WebPluginImpl::RoutingStatus WebPluginImpl::RouteToFrame(
  ...
  request.setSkipServiceWorker(blink::WebURLRequest::SkipServiceWorker::All);
  if (len > 0) {
...
#if defined(OS_WEBOS)
void WebPluginImpl::PluginEnabledRendering() {}
...
void WebPluginImpl::AcceleratedPluginEnabledRendering(bool paintless) {
  if (paintless) {
    if (!container_)
      return;
    if (!solid_color_layer_.get()) {
      solid_color_layer_ = cc::SolidColorLayer::Create();
      web_layer_.reset(new cc_blink::WebLayerImpl(solid_color_layer_));
      container_->setWebLayer(web_layer_.get());
    }
    ...
    layer_bridge_ = WebPluginLayerBridge::Create(this);
  }
}

void WebPluginImpl::AcceleratedPluginAllocateBuffer(
    uint32_t width,
    uint32_t height,
  ...
  if (layer_bridge_.get())
    layer_bridge_->AllocateBuffer(width, height, internalformat, usage, format,
                                  id, handle);
}

void WebPluginImpl::AcceleratedPluginReleaseBuffer(int32_t id) {
  if (layer_bridge_.get())
    layer_bridge_->ReleaseBuffer(id);
}

void WebPluginImpl::AcceleratedPluginSwapBuffer(int32_t id) {
  if (!container_)
    return;

  int32_t next_swap_buffer_id = layer_bridge_.get() ? id : 0;

  if (next_swap_buffer_id) {
    if (!texture_layer_.get()) {
      texture_layer_ = cc::TextureLayer::CreateForMailbox(layer_bridge_.get());
      texture_layer_->SetIsDrawable(true);
      // Upper and left edges of the content at top-left texture origin (0, 0).
    ...
    if (layer_bridge_.get())
      layer_bridge_->SwapBuffer(next_swap_buffer_id);
  } else {
    container_->setWebLayer(NULL);
    web_layer_.reset();
    texture_layer_ = NULL;
...
void WebPluginImpl::AcceleratedPluginDidSwapBuffer(int32_t id) {
  if (delegate_)
    delegate_->AcceleratedPluginDidSwapBuffer(id);
}

void WebPluginImpl::AcceleratedPluginDidSwapBufferComplete(int32_t id) {
  if (delegate_)
    delegate_->AcceleratedPluginDidSwapBufferComplete(id);
}

void WebPluginImpl::SetUsesPunchHoleForVideo(bool uses_punch_hole_for_video) {
  uses_punch_hole_for_video_ = uses_punch_hole_for_video;
  if (texture_layer_.get()) {
    texture_layer_->SetUsesPunchHoleForVideo(uses_punch_hole_for_video_);
  } else
...
void WebPluginImpl::HandleURLRequestInternal(const char* url,
  ...
  if (!base::CommandLine::ForCurrentProcess()->HasSwitch(
    ...
    SetReferrer(&request, referrer_flag);
    Referrer referrer(
        blink::WebStringToGURL(request.httpHeaderField(WebString::fromUTF8("Referer"))),
...
bool WebPluginImpl::InitiateHTTPRequest(unsigned long resource_id,
  ...
  // ServiceWorker is disabled for NPAPI.
  info.request.setSkipServiceWorker(blink::WebURLRequest::SkipServiceWorker::All);
...
#if defined(OS_WEBOS)
  // Begin layer bridge destruction to allow release shared buffer mailboxes.
  if (layer_bridge_.get())
