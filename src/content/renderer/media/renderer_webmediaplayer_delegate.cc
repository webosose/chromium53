...
#include "content/public/common/screen_orientation_type.h"
...
#if defined(OS_WEBOS) && defined(USE_UMEDIASERVER)
gfx::Rect RendererWebMediaPlayerDelegate::GetRenderViewBounds() {
  return render_frame()->GetRenderView()->GetWidget()->windowRect();
}
#endif
...
bool RendererWebMediaPlayerDelegate::OnMessageReceived(
  ...
  IPC_BEGIN_MESSAGE_MAP(RendererWebMediaPlayerDelegate, msg)
    ...
    IPC_MESSAGE_HANDLER(MediaPlayerDelegateMsg_UpdateScreenOrientation,
                        OnMediaDelegateScreenOrientationUpdate)
...
void RendererWebMediaPlayerDelegate::OnMediaDelegateScreenOrientationUpdate(
    int delegate_id,
    int screen_orientation) {
  Observer* observer = id_map_.Lookup(delegate_id);
  if (observer) {
    media::ScreenOrientationType media_screen_orientation;
    switch (screen_orientation) {
      case content::CONTENT_SCREEN_ORIENTATION_UNDEFINED:
        media_screen_orientation = media::MEDIA_SCREEN_ORIENTATION_UNDEFINED;
        break;
      case content::CONTENT_SCREEN_ORIENTATION_0:
        media_screen_orientation = media::MEDIA_SCREEN_ORIENTATION_0;
        break;
      case content::CONTENT_SCREEN_ORIENTATION_90:
        media_screen_orientation = media::MEDIA_SCREEN_ORIENTATION_90;
        break;
      case content::CONTENT_SCREEN_ORIENTATION_180:
        media_screen_orientation = media::MEDIA_SCREEN_ORIENTATION_180;
        break;
      case content::CONTENT_SCREEN_ORIENTATION_270:
        media_screen_orientation = media::MEDIA_SCREEN_ORIENTATION_270;
        break;
      default:
        media_screen_orientation =
            (media::ScreenOrientationType)screen_orientation;
    };
    observer->OnScreenOrientationUpdate(media_screen_orientation);
  }
}
