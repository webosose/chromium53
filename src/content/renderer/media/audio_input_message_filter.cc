...
void AudioInputMessageFilter::SuspendDevices(bool suspend) {
  DCHECK(io_task_runner_->BelongsToCurrentThread());

  IDMap<media::AudioInputIPCDelegate>::iterator it(&delegates_);
  while (!it.IsAtEnd()) {
    int stream_id = it.GetCurrentKey();
    if (suspend) {
      Send(new AudioInputHostMsg_PauseStream(stream_id));
    } else {
      Send(new AudioInputHostMsg_ResumeStream(stream_id));
    }
    it.Advance();
  }
}
