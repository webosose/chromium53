...
class CONTENT_EXPORT RenderThreadImpl
 ...
 public:
  ...
  scoped_refptr<ContextProviderCommandBuffer>
  SharedMainThreadPluginContextProvider();
 ...
 private:
...
#endif
#if defined(OS_WEBOS)
  void OnNotifyLocaleChanged(const std::string& new_locale);
#endif
  ...
  // maximum skia font cache size for background tab in idle. (default: -1 = don't check)
  int skia_background_font_cache_size_;
  ...
  scoped_refptr<ContextProviderCommandBuffer>
      shared_main_thread_plugin_contexts_;
