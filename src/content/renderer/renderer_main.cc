...
int RendererMain(const MainFunctionParams& parameters) {
  ...
  if (parsed_command_line.HasSwitch(switches::kSkiaImageCacheSizeMb)) {
    const int kMB = 1024 * 1024;
    size_t cache_size;
    if (base::StringToSizeT(
        parsed_command_line.GetSwitchValueNative(switches::kSkiaImageCacheSizeMb),
        &cache_size))
      SkGraphics::SetResourceCacheTotalByteLimit(cache_size * kMB);
  }

  if (parsed_command_line.HasSwitch(switches::kSkiaFontCacheSizeMb)) {
    const int kMB = 1024 * 1024;
    size_t cache_size;
    if (base::StringToSizeT(
        parsed_command_line.GetSwitchValueNative(switches::kSkiaFontCacheSizeMb),
        &cache_size))
      SkGraphics::SetFontCacheLimit(cache_size * kMB);
  }
