...
#if defined(OS_WEBOS)
const size_t kExtraCharsBeforeAndAfterSelection = 512;
#else
const size_t kExtraCharsBeforeAndAfterSelection = 100;
#endif

#if defined(OS_WEBOS) && defined(USE_UMEDIASERVER)
const char kPrefixOfWebOSMimeType[] = "service/webos-*";
#endif
...
// static
void RenderFrameImpl::DispatchCustomEvent(const std::string& eventName) {
  v8::Isolate* isolate = v8::Isolate::GetCurrent();
  v8::HandleScope handle_scope(isolate);

  for (FrameMap::iterator it = g_frame_map.Get().begin();
       it != g_frame_map.Get().end(); ++it) {
    (it->first)->document().dispatchEvent(
        WebDOMCustomEvent(WebString::fromUTF8(eventName)));
  }
}
...
void RenderFrameImpl::OnCSSInjectRequest(const std::string& css) {
  ...
  frame_->view()->injectStyleSheet(
      WebString::fromUTF8(css.c_str()), white, WebView::InjectStyleInAllFrames);
}
...
void RenderFrameImpl::OnSetAccessibilityMode(AccessibilityMode new_mode) {
  ...
  if (accessibility_mode_ == AccessibilityModeOff) {
    render_view()->GetWebView()->settings()->setAccessibilityExploreByMouseEnabled(false);
    return;
  }
  ...
  if (accessibility_mode_ & AccessibilityModeFlagFullTree) {
    ...
    if (base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kWebOSWAM))
      RenderFrameImpl::DispatchCustomEvent("webOSAccessibilityReady");
#endif
  }

  if (accessibility_mode_ & AccessibilityModeComplete) {
    if (GetWebkitPreferences().accessibility_explore_by_mouse_enabled)
      render_view()->GetWebView()->settings()->setAccessibilityExploreByMouseEnabled(true);
  }
}
...
#endif  // defined(OS_WEBOS) && defined(USE_UMEDIASERVER)
...
void RenderFrameImpl::frameFocused() {
  render_view_->didFirstFrameFocused();
...
void RenderFrameImpl::didCreateDataSource(blink::WebLocalFrame* frame,
  ...
  if (content_initiated) {
    ...
    switch (request.getCachePolicy()) {
      ...
      case WebCachePolicy::PreloadControlDisableCache:  // preload
...
void RenderFrameImpl::didChangeSelection(bool is_empty_selection) {
    if (is_empty_selection &&
      ...
      if (!GetRenderWidget()->webwidget()->caretOrSelectionRange(&location,
                                                                 &length))
        return;

      gfx::Range range = gfx::Range(location, location + length);

      base::string16 text =
          WebRange::fromDocumentRange(frame_, 0,
                                      kExtraCharsBeforeAndAfterSelection + 1)
              .toPlainText();
...
void RenderFrameImpl::willSendRequest(
  ...
  if (render_view_->block_write_diskcache())
    request.setCachePolicy(WebCachePolicy::PreloadControlDisableCache);
...
WebNavigationPolicy RenderFrameImpl::decidePolicyForNavigation(
  ...
  if (!frame_->parent() && is_content_initiated &&
    ...
    if (!should_fork && url.SchemeIs(url::kFileScheme)) {
      ...
      // If try to access local file from remote url, then check settings related with
      // If webSecurityEnable-false, allowLocalResourceLoad-true
      // then local resource access should be allowed
      WebSettings* settings = render_view_->webview()->settings();
      if(!source_url.SchemeIs(url::kFileScheme) &&
          settings->webSecurityEnabled() &&
          !settings->allowLocalResourceLoad()) {
...
void RenderFrameImpl::didHistoryBackOnTopPage(blink::WebFrame* frame) {
  if (frame != render_view_->webview()->mainFrame())
    return;
  Send(new FrameHostMsg_DidHistoryBackOnTopPage(routing_id_));
}

std::string RenderFrameImpl::GetWebContentUserAgent() {
  std::string res;
  Send(new FrameHostMsg_WebContentUserAgent(routing_id_, &res));
  return res;
}
