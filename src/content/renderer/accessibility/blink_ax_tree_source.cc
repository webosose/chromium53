...
void BlinkAXTreeSource::SerializeNode(blink::WebAXObject src,
  ...
  if (!node.isNull() && node.isElementNode()) {
    ...
    // Set as -1 if aria-setsize attribute is not defined
    if (element.hasAttribute("aria-setsize")) {
      if (src.setSize())
        dst->AddIntAttribute(ui::AX_ATTR_SET_SIZE, src.setSize());
    } else
      dst->AddIntAttribute(ui::AX_ATTR_SET_SIZE, -1);

    // Set as -1 if aria-posinset attribute is not defined
    if (element.hasAttribute("aria-posinset")) {
      if (src.posInSet())
        dst->AddIntAttribute(ui::AX_ATTR_POS_IN_SET, src.posInSet());
    } else
      dst->AddIntAttribute(ui::AX_ATTR_POS_IN_SET, -1);
