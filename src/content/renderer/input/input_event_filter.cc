...
void InputEventFilter::AdjustGestureScrollDeltaIfNativeScrollEnabled(
    const blink::WebInputEvent* event) {
  if (base::CommandLine::ForCurrentProcess()->HasSwitch(
          switches::kEnableWebOSNativeScroll) &&
    ...
    blink::WebGestureEvent* gesture_event =
        static_cast<blink::WebGestureEvent*>(
            const_cast<blink::WebInputEvent*>(event));

    if (gesture_event->data.scrollUpdate.generatedFromMouseWheelEvent) {
      ...
      int converted_scroll_delta_x = (gesture_event->data.scrollUpdate.deltaX /
                                      ui::MouseWheelEvent::kWheelDelta) *
                                      custom_delta;
      int converted_scroll_delta_y = (gesture_event->data.scrollUpdate.deltaY /
                                      ui::MouseWheelEvent::kWheelDelta) *
                                      custom_delta;

      gesture_event->data.scrollUpdate.deltaX = converted_scroll_delta_x;
      gesture_event->data.scrollUpdate.deltaY = converted_scroll_delta_y;
    }
  }
}
...
void InputEventFilter::ForwardToHandler(const IPC::Message& message) {
  ...
  // This is for meeting UX requirement for native scroll based web application.
  // We need to have a method for changing default scroll distance on mouse
  // wheel scrolling.
  // Adjusting scroll delta in this point seems proper way. Because adjusted
  // event is propagated to compositor thread and main thread. So we can cover
  // any scrolling scenarios. But because this class depends on threaded
  // compositing, we need to an assumption that threaded compositing is always
  // enabled. See RenderThreadImpl::InitializeWebKit().
  AdjustGestureScrollDeltaIfNativeScrollEnabled(event);
