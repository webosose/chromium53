...
class RendererBlinkPlatformImpl::MimeRegistry
 ...
 public:
  ...
  blink::WebMimeRegistry::SupportsType supportsMediaSourceMIMEType(const blink::WebString& mime_type,
                                                                   const blink::WebString& codecs,
                                                                   const blink::WebString& width,
                                                                   const blink::WebString& height,
                                                                   const blink::WebString& frame_rate,
                                                                   const blink::WebString& bit_rate,
                                                                   const blink::WebString& channels) override;
#else
  blink::WebMimeRegistry::SupportsType supportsMediaSourceMIMEType(const blink::WebString& mime_type,
                                                                   const blink::WebString& codecs) override;
#endif
...
#if defined(OS_WEBOS) && defined(USE_UMEDIASERVER)
WebMimeRegistry::SupportsType RendererBlinkPlatformImpl::MimeRegistry::supportsMediaSourceMIMEType(
    const blink::WebString& mime_type,
    const WebString& codecs,
    const blink::WebString& width,
    const blink::WebString& height,
    const blink::WebString& frame_rate,
    const blink::WebString& bit_rate,
    const blink::WebString& channels) {
#else
WebMimeRegistry::SupportsType RendererBlinkPlatformImpl::MimeRegistry::supportsMediaSourceMIMEType(
    const blink::WebString& mime_type,
    const WebString& codecs) {
...
#if defined(OS_WEBOS) && defined(USE_UMEDIASERVER)
  const std::string width_ascii = ToASCIIOrEmpty(width);
  const std::string height_ascii = ToASCIIOrEmpty(height);
  const std::string frame_rate_ascii = ToASCIIOrEmpty(frame_rate);
  const std::string bit_rate_ascii = ToASCIIOrEmpty(bit_rate);
  const std::string channels_ascii = ToASCIIOrEmpty(channels);

  return media::StreamParserFactory::IsTypeSupported(
      mime_type_ascii, parsed_codec_ids, width_ascii, height_ascii,
      frame_rate_ascii, bit_rate_ascii, channels_ascii)
...
bool RendererBlinkPlatformImpl::isAggressiveReleasePolicyEnabled() {
  RenderThreadImpl* thread = RenderThreadImpl::current();
  // thread can be NULL in tests.
  return thread && thread->IsAggressiveReleasePolicyEnabled();
}
