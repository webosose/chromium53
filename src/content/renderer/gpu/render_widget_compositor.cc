...
cc::ManagedMemoryPolicy RenderWidgetCompositor::GetGpuMemoryPolicy(
  ...
  size_t webos_system_memory_mb =
      base::GetSystemMemoryInfo(&info) ? info.total / 1024 : 1024;
  if (webos_system_memory_mb >= 1152)
    actual.bytes_limit_when_visible = webos_system_memory_mb / 8;
  else if (webos_system_memory_mb >= 768)
    actual.bytes_limit_when_visible = webos_system_memory_mb / 10;
  else
    actual.bytes_limit_when_visible = webos_system_memory_mb / 12;
  actual.bytes_limit_when_visible =
      actual.bytes_limit_when_visible * 1024 * 1024;
  actual.priority_cutoff_when_visible =
