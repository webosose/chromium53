...
void RenderWidget::UpdateVisualState() {
  ...
  if (webwidget_->isWebView() &&
    ...
    blink::WebFrame* frame =
        reinterpret_cast<blink::WebView*>(webwidget_)->mainFrame();
    if (frame) {
      meaningful_paint_detected_ =
          frame->performance().firstMeaningfulPaintCandidate();
    }
  }
#endif
}
...
void RenderWidget::UpdateSelectionBounds(bool force_update) {
  ...
  if (send_ipc) {
    ...
    if (force_update || selection_anchor_rect_ != params.anchor_rect ||
