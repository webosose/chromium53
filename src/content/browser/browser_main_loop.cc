...
int BrowserMainLoop::BrowserThreadsStarted() {
  ...
  if(!BrowserContext::IsFileAccessBlocked())
  {
    // For web browser which is not in snapshot so File access is blocked, TimeZoneMonitor can be created now
    // For WebAppManager which is in snapshot, TimeZoneMonitor can not be created now,
    // should delay it
    TRACE_EVENT0("startup",
                 "BrowserMainLoop::BrowserThreadsStarted::TimeZoneMonitor; web browser");
...
void BrowserMainLoop::CreateTimeZoneMonitor()
{
  if(time_zone_monitor_.get() == NULL) {
    TRACE_EVENT0("startup",
                 "BrowserMainLoop::CreateTimeZoneMonitor::TimeZoneMonitor Manually");
    time_zone_monitor_ = TimeZoneMonitor::Create();
  }
}
