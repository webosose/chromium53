...
#if defined(OS_WEBOS)
bool RenderFrameHostImpl::HasFocus() {
  if (render_view_host_)
    return render_view_host_->GetWidget()->is_focused();
  return false;
}
#endif
...
void RenderFrameHostImpl::OnDecidePolicyForResponse(
    bool isMainFrame, int statusCode, const GURL& url,
    const base::string16& statusText, bool* hasPolicy) {
  *hasPolicy = delegate_->DecidePolicyForResponse(isMainFrame, statusCode, url, statusText);
}
