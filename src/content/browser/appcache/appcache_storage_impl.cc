...
AppCacheStorageImpl::StoreGroupAndCacheTask::StoreGroupAndCacheTask(
  ...
  origin_limit_size_ = -1;
  base::CommandLine& command_line = *base::CommandLine::ForCurrentProcess();
  if (command_line.HasSwitch(switches::kAppCacheSizePerDomain)) {
      std::string maxAppCacheSizePerDomainString =
          command_line.GetSwitchValueASCII(switches::kAppCacheSizePerDomain);
      int64_t value = 0;
      if (base::StringToInt64(maxAppCacheSizePerDomainString, &value))
          origin_limit_size_ = value;
  }
}
...
void AppCacheStorageImpl::StoreGroupAndCacheTask::Run() {
  ...
  if (space_available_ == -1) {
    int64_t limit_size =
        (origin_limit_size_ > 0) ? origin_limit_size_ : kDefaultQuota;
    if (new_origin_usage_ > limit_size) {
  ...
  // Check the limit size for each origin.
  if (origin_limit_size_ > 0 && new_origin_usage_ > origin_limit_size_) {
    would_exceed_quota_ = true;
    success_ = false;
    return;
  }
...
AppCacheDiskCache* AppCacheStorageImpl::disk_cache() {
  ...
  if (!disk_cache_) {
    ...
    if (is_incognito_) {
      ...
      static int maxDiskCacheSize = 0;
      if (maxDiskCacheSize == 0) {
        maxDiskCacheSize = kMaxDiskCacheSize;
        base::CommandLine& command_line = *base::CommandLine::ForCurrentProcess();
        if (command_line.HasSwitch(switches::kApplicationCacheSize)) {
          std::string maxDiskCacheSizeString = command_line.GetSwitchValueASCII(
              switches::kApplicationCacheSize);
          int value = 0;
          if (base::StringToInt(maxDiskCacheSizeString, &value))
