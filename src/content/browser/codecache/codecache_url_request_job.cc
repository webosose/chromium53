...
void CodeCacheURLRequestJob::Start() {
  ...
  file_task_runner_->PostTaskAndReply(
      FROM_HERE,
      base::Bind(&CodeCacheURLRequestJob::FetchMetaInfo, file_path_,
...
int CodeCacheURLRequestJob::ReadRawData(IOBuffer* dest, int dest_size) {
  ...
  int rv = stream_->Read(
      ...
      base::Bind(&CodeCacheURLRequestJob::DidRead,
                 weak_ptr_factory_.GetWeakPtr(), make_scoped_refptr(dest)));
  if (rv >= 0) {
...
std::unique_ptr<Filter> CodeCacheURLRequestJob::SetupFilter() const {
