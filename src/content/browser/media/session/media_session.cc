...
bool MediaSession::AddPlayer(MediaSessionObserver* observer,
  ...
  observer->OnScreenOrientationChanged(player_id, screen_orientation_);
...
void MediaSession::ScreenOrientationChanged(
    ScreenOrientationType screen_orientation) {
  OnScreenOrientationChangeInternal(screen_orientation);
}
...
void MediaSession::OnScreenOrientationChangeInternal(
    ScreenOrientationType screen_orientation) {
  if (screen_orientation_ == screen_orientation)
    return;

  SetScreenOrientation(screen_orientation);

  for (const auto& it : players_)
    it.observer->OnScreenOrientationChanged(it.player_id, screen_orientation_);
...
MediaSession::MediaSession(WebContents* web_contents)
    : WebContentsObserver(web_contents),
      ...
      screen_orientation_(CONTENT_SCREEN_ORIENTATION_0) {}
...
void MediaSession::SetScreenOrientation(
    ScreenOrientationType screen_orientation) {
  if (screen_orientation == screen_orientation_)
    return;
