...
void MediaSessionController::OnScreenOrientationChanged(
    int player_id,
    ScreenOrientationType screen_orientation) {
  DCHECK_EQ(player_id_, player_id);
  media_web_contents_observer_->Send(
      new MediaPlayerDelegateMsg_UpdateScreenOrientation(
          id_.first->GetRoutingID(), id_.second, screen_orientation));
}
