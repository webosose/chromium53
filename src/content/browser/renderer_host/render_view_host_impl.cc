...
WebPreferences RenderViewHostImpl::ComputeWebkitPrefs() {
  ...
  prefs.css_navigation_enabled = command_line.HasSwitch(
              switches::kEnableCSSNavigation);
...
void RenderViewHostImpl::SetBlockWriteDiskcache(bool blocked) {
  Send(new ViewMsg_SetBlockWriteDiskcache(GetRoutingID(), blocked));
}

void RenderViewHostImpl::ResetStateToMarkNextPaintForContainer() {
  Send(new ViewMsg_ResetStateToMarkNextPaintForContainer(GetRoutingID()));
}
...
bool RenderViewHostImpl::OnMessageReceived(const IPC::Message& msg) {
  ...
  IPC_BEGIN_MESSAGE_MAP(RenderViewHostImpl, msg)
    ...
    IPC_MESSAGE_HANDLER(ViewHostMsg_DidFirstFrameFocused, OnDidFirstFrameFocused)
...
void RenderViewHostImpl::OnClose() {
  ...
  if (GetWebkitPreferences().keep_alive_webapp) {
    ...
    // Just notify about this 'trying close' without any setting for real view closing
    delegate_->Close(this);
    return;
  }
#endif
...
void RenderViewHostImpl::ResumeMedia() {
  Send(new ViewMsg_ResumeMedia(GetRoutingID()));
}

void RenderViewHostImpl::SuspendDOM() {
  Send(new ViewMsg_SuspendDOM(GetRoutingID()));
}

void RenderViewHostImpl::ResumeDOM() {
  Send(new ViewMsg_ResumeDOM(GetRoutingID()));
}

void RenderViewHostImpl::ContentsPositionChanged(float position) {
  Send(new ViewMsg_ContentsPositionChanged(GetRoutingID(), position));
}

void RenderViewHostImpl::OnDidFirstFrameFocused() {
