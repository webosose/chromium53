...
class CONTENT_EXPORT AudioInputRendererHost
 ...
 public:
  ...
  void OnPaused(media::AudioInputController* controller) override {}
  void OnResumed(media::AudioInputController* controller) override {}
 ...
 private:
  ...
  // Pause capturing the audio input stream referenced by |stream_id|.
  void OnPauseStream(int stream_id);

  // Resume after pause of the audio input stream referenced by |stream_id|.
  void OnResumeStream(int stream_id);
