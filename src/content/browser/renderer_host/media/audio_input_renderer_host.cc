...
bool AudioInputRendererHost::OnMessageReceived(const IPC::Message& message) {
  ...
  IPC_BEGIN_MESSAGE_MAP(AudioInputRendererHost, message)
    ...
    IPC_MESSAGE_HANDLER(AudioInputHostMsg_PauseStream, OnPauseStream)
    IPC_MESSAGE_HANDLER(AudioInputHostMsg_ResumeStream, OnResumeStream)
...
void AudioInputRendererHost::OnPauseStream(int stream_id) {
  DCHECK_CURRENTLY_ON(BrowserThread::IO);
  LogMessage(stream_id, "OnPauseStream", true);
  ...
  audio_log_->OnPaused(stream_id);
}

void AudioInputRendererHost::OnResumeStream(int stream_id) {
  DCHECK_CURRENTLY_ON(BrowserThread::IO);
  LogMessage(stream_id, "OnResumeStream", true);
  ...
  audio_log_->OnResumed(stream_id);
}
