...
void DelegatedFrameHost::WasShown(const ui::LatencyInfo& latency_info) {
  ...
  if (compositor_) {
    ...
    if (use_aggresssive_release_policy_) {
      if (!deferred_resume_drawing_ && did_first_swap_ && was_hidden_) {
...
#if defined(OS_WEBOS)
void DelegatedFrameHost::ForceReclaimResources() {
  if (!surface_id_.is_null() && surface_factory_.get()) {
    cc::SurfaceFactory::DrawCallback ack_callback;
    if (compositor_) {
      ack_callback = base::Bind(&DelegatedFrameHost::ForceReclaimResourcesAck,
                                AsWeakPtr(), last_output_surface_id_);
    }
    surface_factory_->SubmitCompositorFrame(surface_id_, cc::CompositorFrame(),
                                            ack_callback);
  }
}
#endif
...
#if defined(OS_WEBOS)
void DelegatedFrameHost::ForceReclaimResourcesAck(uint32_t output_surface_id,
