...
RenderWidgetHostViewAura::RenderWidgetHostViewAura(RenderWidgetHost* host,
    ...
    : host_(RenderWidgetHostImpl::From(host)),
      ...
      enable_html_systemkeyboard_attr_(false),
      window_scale_ratio_(1.0f),
#endif
      is_guest_view_hack_(is_guest_view_hack),
      set_focus_on_mouse_down_or_key_event_(false),
      device_scale_factor_(0.0f),
      disable_input_event_router_for_testing_(false),
      ime_restorable_(true),
...
#if defined(OS_WEBOS)
  window_->SetType(ui::wm::WINDOW_TYPE_POPUP);
#else
...
void RenderWidgetHostViewAura::RenderProcessGone(base::TerminationStatus status,
                                                 int error_code) {
  if (GetInputMethod()) {
    GetInputMethod()->HideIme(ui::IME_CLOSE);
  }
  UpdateCursorIfOverSelf();
  Destroy();
}
...
void RenderWidgetHostViewAura::GetScreenInfo(WebScreenInfo* results) {
  ...
  if (!hardware_resolution_.isEmpty())
    results->hardwareResolution = hardware_resolution_;
#endif
}
...
#if defined(OS_WEBOS)
  host_->ImeConfirmComposition(base::string16(), range, false);
  host_->ExecuteEditCommand("DeleteBackward", "");
  return true;
#else
  // TODO(suzhe): implement this method when fixing http://crbug.com/55130.
  NOTIMPLEMENTED();
...
bool RenderWidgetHostViewAura::IsImeRestorable() {
  return ime_restorable_;
}
...
void RenderWidgetHostViewAura::OnKeyEvent(ui::KeyEvent* event) {
  ...
  if (is_fullscreen_ && event->key_code() == ui::VKEY_ESCAPE) {
    ...
    if (has_composition_text_ && (event->key_code() == ui::VKEY_LEFT ||
...
#if defined(OS_WEBOS)
void RenderWidgetHostViewAura::ScrollView(
    const blink::WebMouseWheelEvent& event) {
  int max_scroll_height =
      -GetInputMethod()->GetInputPanelRect().height() / window_scale_ratio_;

  gfx::Rect bounds = window_->bounds();

  RenderViewHost* rvh = RenderViewHost::From(host_);
  rvh->ContentsPositionChanged(
      (static_cast<int>(event.deltaY) > 0)
          ? std::min(bounds.y() + static_cast<int>(event.deltaY), 0)
          : std::max(bounds.y() + static_cast<int>(event.deltaY),
                     max_scroll_height));

  window_->SetBounds(gfx::Rect(
      bounds.x(), (static_cast<int>(event.deltaY) > 0)
                      ? std::min(bounds.y() + static_cast<int>(event.deltaY), 0)
                      : std::max(bounds.y() + static_cast<int>(event.deltaY),
                                 max_scroll_height),
...
void RenderWidgetHostViewAura::OnUpdateTextInputStateCalled(
  ...
  if (state) {
#if defined(OS_WEBOS)
    if (enable_html_systemkeyboard_attr_ &&
        (state->flags & blink::WebTextInputFlagSystemKeyboardOff))
      return;
#endif
...
#if defined(OS_WEBOS)
void RenderWidgetHostViewAura::SetHardwareResolution(int width, int height) {
  hardware_resolution_ = blink::WebSize(width, height);
}
