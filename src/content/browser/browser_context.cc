...
bool BrowserContext::file_access_blocked_ = false;
void BrowserContext::SetFileAccessBlocked(bool blocked) {
  if(!blocked && file_access_blocked_ != blocked)
    BrowserMainLoop::GetInstance()->CreateTimeZoneMonitor();

  file_access_blocked_ = blocked;
}

bool BrowserContext::IsFileAccessBlocked() {
  return file_access_blocked_;
}
}  // namespace content
