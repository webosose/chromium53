...
bool WebContentsImpl::OnMessageReceived(RenderViewHost* render_view_host,
  ...
  IPC_BEGIN_MESSAGE_MAP(WebContentsImpl, message)
    ...
    IPC_MESSAGE_HANDLER(FrameHostMsg_WebContentUserAgent, OnWebContentUserAgent)
  IPC_END_MESSAGE_MAP()
  render_view_message_source_ = NULL;
  render_frame_message_source_ = NULL;
...
void WebContentsImpl::EnableCompleteAccessibilityMode() {
  AddAccessibilityMode(AccessibilityModeComplete);
}

void WebContentsImpl::DisableAccessibilityMode() {
...
const base::string16& WebContentsImpl::GetDocumentTitle() const {
...
void WebContentsImpl::OnDidDropAllPeerConnections(
  ...
  FOR_EACH_OBSERVER(WebContentsObserver, observers_,
                    DidDropAllPeerConnections(reason));
}
...
void WebContentsImpl::OnWillSwapMeaningfulPaint(double detected_time) {
  FOR_EACH_OBSERVER(WebContentsObserver, observers_,
                    WillSwapMeaningfulPaint(detected_time));
}
...
void WebContentsImpl::DidFirstFrameFocused() {
  if (delegate_)
...
void WebContentsImpl::AddAvailablePluginDir(const std::string& directory) {
  available_plugin_dirs_.push_back(directory);
}

const std::vector<std::string>& WebContentsImpl::GetAvailablePluginDirs() const {
  return available_plugin_dirs_;
}
...
void WebContentsImpl::RenderProcessCreated(RenderProcessHost* render_process_host)
{
  FOR_EACH_OBSERVER(WebContentsObserver,
                    observers_,
                    RenderProcessCreated(render_process_host->GetHandle()));
}

bool WebContentsImpl::DecidePolicyForResponse(
    bool isMainFrame, int statusCode,
    const GURL& url,
    const base::string16& statusText) {
  if (delegate_)
    return delegate_->DecidePolicyForResponse(isMainFrame, statusCode, url, statusText);
  return false;
}

void WebContentsImpl::OnWebContentUserAgent(std::string* res) {
  *res = this->GetUserAgentOverride();
  if (res->empty())
    *res = content::GetContentClient()->GetUserAgent();
}
