...
#endif
#if defined(OS_WEBOS)
    IPC_MESSAGE_HANDLER(PluginMsg_Suspend, OnSuspend)
    IPC_MESSAGE_HANDLER(PluginMsg_Resume, OnResume)
    IPC_MESSAGE_HANDLER(PluginMsg_SetDeviceInfo, OnSetDeviceInfo)
    IPC_MESSAGE_HANDLER(PluginMsg_AcceleratedPluginDidSwapBuffer,
                        OnAcceleratedPluginDidSwapBuffer)
    IPC_MESSAGE_HANDLER(PluginMsg_AcceleratedPluginDidSwapBufferComplete,
                        OnAcceleratedPluginDidSwapBufferComplete)
#endif
...
#if defined(OS_WEBOS)
void WebPluginDelegateStub::OnSuspend() {
  delegate_->Suspend();
}

void WebPluginDelegateStub::OnResume() {
  delegate_->Resume();
}

void WebPluginDelegateStub::OnSetDeviceInfo(
    const gfx::Size& device_viewport_size,
    const gfx::Size& view_size) {
  delegate_->SetDeviceInfo(device_viewport_size, view_size);
}

void WebPluginDelegateStub::OnAcceleratedPluginDidSwapBuffer(int32_t id) {
  webplugin_->AcceleratedPluginDidSwapBuffer(id);
}

void WebPluginDelegateStub::OnAcceleratedPluginDidSwapBufferComplete(
    int32_t id) {
  webplugin_->AcceleratedPluginDidSwapBufferComplete(id);
}
#endif
