...
void WebPluginProxy::InvalidateRect(const gfx::Rect& rect) {
  ...
  if (accelerated_surface_) {
    accelerated_surface_->InvalidateRect(rect);
    return;
  }
...
void WebPluginProxy::CreateDIBAndCanvasFromHandle(
  ...
  // Convert the shared memory handle to a handle that works in our process,
  // and then use that to create a SkCanvas.
  TransportDIB* dib = TransportDIB::Map(dib_handle);
...
void WebPluginProxy::SetWindowlessBuffers(
    ...
    const TransportDIB::Handle& windowless_buffer1,
    const gfx::Rect& window_rect) {
  CreateDIBAndCanvasFromHandle(windowless_buffer0, window_rect,
                               &windowless_dibs_[0], &windowless_canvases_[0]);
  if (!windowless_canvases_[0]) {
    windowless_canvases_[1].reset();
    return;
  }
  CreateDIBAndCanvasFromHandle(windowless_buffer1, window_rect,
                               &windowless_dibs_[1], &windowless_canvases_[1]);
  if (!windowless_canvases_[1]) {
...
#if defined(OS_WEBOS)
void WebPluginProxy::PluginEnabledRendering() {
  Send(new PluginHostMsg_PluginEnabledRendering(route_id_));
}
...
void WebPluginProxy::AcceleratedPluginEnabledRendering(bool paintless) {
  Send(new PluginHostMsg_AcceleratedPluginEnabledRendering(route_id_,
                                                           paintless));
}

void WebPluginProxy::AcceleratedPluginAllocateBuffer(
    uint32_t width,
    uint32_t height,
  ...
  Send(new PluginHostMsg_AcceleratedPluginAllocateBuffer(
      route_id_, width, height, internalformat, usage, format, id, handle));
}

void WebPluginProxy::AcceleratedPluginReleaseBuffer(int32_t id) {
  Send(new PluginHostMsg_AcceleratedPluginReleaseBuffer(route_id_, id));
}

void WebPluginProxy::AcceleratedPluginSwapBuffer(int32_t id) {
  Send(new PluginHostMsg_AcceleratedPluginSwapBuffer(route_id_, id));
}

void WebPluginProxy::AcceleratedPluginDidSwapBuffer(int32_t id) {
  if (accelerated_surface_)
    accelerated_surface_->DidSwapBuffer(id);
}

void WebPluginProxy::AcceleratedPluginDidSwapBufferComplete(int32_t id) {
  if (accelerated_surface_)
    accelerated_surface_->DidSwapBufferComplete(id);
}

void WebPluginProxy::SetUsesPunchHoleForVideo(bool uses_punch_hole_for_video) {
  Send(new PluginHostMsg_SetUsesPunchHoleForVideo(route_id_,
                                                  uses_punch_hole_for_video));
}
#endif
...
std::string WebPluginProxy::GetUserAgent() {
  std::string res;
  Send(new PluginHostMsg_GetUserAgent(route_id_, &res));
  return res;
}
