...
WebMimeRegistry::SupportsType SimpleWebMimeRegistryImpl::supportsMediaSourceMIMEType(
    const WebString& mime_type,
    const WebString& codecs) {
  // Media features are only supported at the content/renderer layer.
...
WebMimeRegistry::SupportsType SimpleWebMimeRegistryImpl::supportsMediaSourceMIMEType(
    const WebString& mime_type,
    const WebString& codecs,
    const WebString& width,
    const WebString& height,
    const WebString& frame_rate,
    const WebString& bit_rate,
    const WebString& channels
    ) {
  // Media features are only supported at the content/renderer layer.
  const std::string mime_type_ascii = ToASCIIOrEmpty(mime_type);
  return media::IsSupportedMediaMimeType(mime_type_ascii)
