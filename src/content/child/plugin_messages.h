...
#if defined(OS_WEBOS)
IPC_MESSAGE_ROUTED0(PluginMsg_Suspend)
IPC_MESSAGE_ROUTED0(PluginMsg_Resume)
IPC_MESSAGE_ROUTED2(PluginMsg_SetDeviceInfo,
                    gfx::Size /* device_viewport_size */,
                    gfx::Size /* view_size */)

// Sent by the renderer after it prepares texture mailbox from
// external texture based on gpu memory buffer so that the
// plugin knows it can paint next frame.
IPC_MESSAGE_ROUTED1(PluginMsg_AcceleratedPluginDidSwapBuffer, int32_t /* id */)

// Sent by the renderer to notify plugin that it can reuse
// gpu memory buffer.
IPC_MESSAGE_ROUTED1(PluginMsg_AcceleratedPluginDidSwapBufferComplete,
                    int32_t /* id */)

// Notifies the renderer process that the plugin has punch hole for video.
IPC_MESSAGE_ROUTED1(PluginHostMsg_SetUsesPunchHoleForVideo,
...
// Ask Render process through WebPlugin proxy about User Agent
IPC_SYNC_MESSAGE_ROUTED0_1(PluginHostMsg_GetUserAgent,
                           std::string /* result */)
...
#if defined(OS_WEBOS)
// Notifies the renderer process that this plugin will be renderable.
IPC_SYNC_MESSAGE_ROUTED0_0(PluginHostMsg_PluginEnabledRendering)

// Notifies the renderer process that this plugin will be using the
// accelerated rendering path.
IPC_SYNC_MESSAGE_ROUTED1_0(PluginHostMsg_AcceleratedPluginEnabledRendering,
                           bool /* paintless */)

// Requests the renderer process to allocate gpu memory buffer for
// accelerated surface into which plugin will render.
IPC_SYNC_MESSAGE_ROUTED5_2(PluginHostMsg_AcceleratedPluginAllocateBuffer,
                           uint32_t /* width */,
                           uint32_t /* height */,
...
// Notifies the renderer process that the plugin released
// gpu memory buffer.
IPC_MESSAGE_ROUTED1(PluginHostMsg_AcceleratedPluginReleaseBuffer,
                    int32_t /* id */)

// Notifies the renderer process that the plugin painted content
// and buffer is ready to swap.
IPC_MESSAGE_ROUTED1(PluginHostMsg_AcceleratedPluginSwapBuffer, int32_t /* id */)
#endif
