...
bool WebPluginDelegateImpl::Initialize(
  ...
  int argc = 0;
  std::unique_ptr<char*[]> argn(new char*[arg_names.size()]);
  std::unique_ptr<char*[]> argv(new char*[arg_names.size()]);
...
void WebPluginDelegateImpl::DidReceiveManualResponse(
    ...
    const std::string& headers, uint32_t expected_length, uint32_t last_modified) {
  if (!windowless_) {
