...
class MultiPartResponseClient : public blink::WebURLLoaderClient {
 public:
  ...
  void didReceiveData(blink::WebURLLoader* loader,
    ...
    int64_t data_offset = byte_range_lower_bound_;
...
class PluginURLFetcher::RequestPeerImpl : public RequestPeer {
 public:
  explicit RequestPeerImpl(PluginURLFetcher* fetcher);
 ...
 private:
  scoped_refptr<PluginURLFetcher> fetcher_;
...
PluginURLFetcher::PluginURLFetcher(PluginStreamUrl* plugin_stream,
  ...
  request_info.skip_service_worker = SkipServiceWorker::ALL;
  ...
  request_id_ = ChildThreadImpl::current()->resource_dispatcher()->StartAsync(
      ...
      base::WrapUnique(new PluginURLFetcher::RequestPeerImpl(this)));
...
void PluginURLFetcher::OnUploadProgress(uint64_t position, uint64_t size) {
}
...
void PluginURLFetcher::OnReceivedResponse(const ResourceResponseInfo& info) {
  ...
  if (info.headers.get()) {  // NULL for data: urls.
    if (info.headers->GetLastModifiedValue(&temp))
      last_modified = static_cast<uint32_t>(temp.ToDoubleT());
...
void PluginURLFetcher::OnReceivedData(
  ...
  if (multipart_delegate_) {
    ...
    if (copy_stream_data_) {
      ...
      std::unique_ptr<char[]> data_copy(new char[data_length]);
...
PluginURLFetcher::RequestPeerImpl::RequestPeerImpl(PluginURLFetcher* fetcher)
    : fetcher_(fetcher) {}

void PluginURLFetcher::RequestPeerImpl::OnUploadProgress(
    uint64_t position, uint64_t size) {
  fetcher_->OnUploadProgress(position, size);
}

bool PluginURLFetcher::RequestPeerImpl::OnReceivedRedirect(
    const net::RedirectInfo& redirect_info,
    const ResourceResponseInfo& info) {
  return fetcher_->OnReceivedRedirect(redirect_info, info);
}

void PluginURLFetcher::RequestPeerImpl::OnReceivedResponse(
    const ResourceResponseInfo& info) {
  fetcher_->OnReceivedResponse(info);
}

void PluginURLFetcher::RequestPeerImpl::OnDownloadedData(
    int len, int encoded_data_length) {
  fetcher_->OnDownloadedData(len, encoded_data_length);
}

void PluginURLFetcher::RequestPeerImpl::OnReceivedData(
    std::unique_ptr<ReceivedData> data) {
  fetcher_->OnReceivedData(std::move(data));
}

void PluginURLFetcher::RequestPeerImpl::OnCompletedRequest(
    int error_code,
    bool was_ignored_by_handler,
    bool stale_copy_in_cache,
    const std::string& security_info,
    const base::TimeTicks& completion_time,
    int64_t total_transfer_size) {
  fetcher_->OnCompletedRequest(error_code,
