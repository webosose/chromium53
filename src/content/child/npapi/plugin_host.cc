...
const char* NPN_UserAgent(NPP id) {
  ...
  if (leaky_user_agent == NULL) {
    ...
    if (plugin.get() && plugin->webplugin()) {
      leaky_user_agent.Get() = plugin->webplugin()->GetUserAgent();
    } else {
...
NPError NPN_SetValue(NPP id, NPPVariable variable, void* value) {
  ...
  #endif
#if defined(OS_WEBOS)
    case NPPVpluginUsesPunchHoleForVideoBool: {
      // Set external video value to draw video hole.
      plugin->set_punch_hole_for_video(value);
      return NPERR_NO_ERROR;
    }
#endif
    default:
