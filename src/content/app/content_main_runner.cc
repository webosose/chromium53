...
#if !defined(OS_LINUX) || defined(WEBOS_PLUGIN_SUPPORT)
...
void SetupSignalHandlers() {
  ...
  static const int signals_to_reset[] = {
      ...
      SIGTERM, SIGCHLD, SIGTRAP};  // SIGPIPE is set below.
  for (unsigned i = 0; i < arraysize(signals_to_reset); i++) {
    CHECK_EQ(0, sigaction(signals_to_reset[i], &sigact, NULL));
  }
#if !defined(OS_WEBOS)
  CHECK_EQ(0, sigaction(SIGILL, &sigact, NULL));
  CHECK_EQ(0, sigaction(SIGABRT, &sigact, NULL));
  CHECK_EQ(0, sigaction(SIGFPE, &sigact, NULL));
  CHECK_EQ(0, sigaction(SIGSEGV, &sigact, NULL));
  CHECK_EQ(0, sigaction(SIGBUS, &sigact, NULL));
#endif
