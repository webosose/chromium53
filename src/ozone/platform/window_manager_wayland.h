...
class WindowManagerWayland
 ...
 private:
  ...
  void OnReinitializeWindow(unsigned handle);
  ...
  void ReinitializeWindow(unsigned handle);
  ...
  void NotifyCompositorBuffersSwapped(unsigned handle);
  ...
  void NotifyKeyboardModifier(uint32_t mods_depressed, uint32_t mods_locked);
  void NotifyNativeWindowExposed(unsigned handle);
  void NotifyNativeWindowStateAboutToChange(unsigned handle,
                                            ui::WidgetState state);
  void NotifyNativeWindowStateChanged(unsigned handle, ui::WidgetState state);
  void NotifyTextInputModifier(uint32_t state, uint32_t modifier);
  void OnWebosWindowClose(unsigned windowhandle);
#endif
