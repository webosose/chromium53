...
void DesktopWindowTreeHostOzone::Init(
  ...
  compositor()->PrepareOutputSurface();
...
ui::InputMethod* DesktopWindowTreeHostOzone::GetInputMethod() {
  return aura::WindowTreeHost::GetInputMethod();
}

unsigned DesktopWindowTreeHostOzone::GetWindowHandle() {
  return GetWindow();
}
...
void DesktopWindowTreeHostOzone::Resize(int width, int height) {
  ...
  compositor()->SetScaleAndSize(device_scale, bounds.size());
  UpdateRootWindowSize(bounds.size());
}

void DesktopWindowTreeHostOzone::ResetCompositor() {
  if (!compositor())
    return;
  compositor()->ResumeDrawing();
}

void DesktopWindowTreeHostOzone::WebOSXInputActivate(const std::string& type) {
  platform_window_->WebOSXInputActivate(type);
}

void DesktopWindowTreeHostOzone::WebOSXInputDeactivate() {
  platform_window_->WebOSXInputDeactivate();
}

void DesktopWindowTreeHostOzone::WebOSXInputInvokeAction(
    uint32_t keysym,
    ui::WebOSXInputSpecialKeySymbolType symbol_type,
    ui::WebOSXInputEventType event_type) {
  platform_window_->WebOSXInputInvokeAction(keysym, symbol_type, event_type);
}

void DesktopWindowTreeHostOzone::SetCustomCursor(webos::CustomCursorType type,
...
void DesktopWindowTreeHostOzone::SetPlatformCursor(
    const SkBitmap* cursor_bitmap,
    const gfx::Point& location) {
  if (platform_cursor_)
    ui::CursorFactoryOzone::GetInstance()->UnrefImageCursor(platform_cursor_);
  platform_cursor_ = ui::CursorFactoryOzone::GetInstance()->CreateImageCursor(
      *cursor_bitmap, location);
  platform_window_->SetCursor(platform_cursor_);
}

void DesktopWindowTreeHostOzone::SetKeyMask(webos::WebOSKeyMask key_mask, bool value) {
  platform_window_->SetKeyMask(key_mask, value);
}

void DesktopWindowTreeHostOzone::SetWindowHostState(ui::WidgetState state) {
  switch (state) {
    case ui::WidgetState::FULLSCREEN:
      // Wayland Window needs non-empty size.
      // If wayland window sets empty size, wayland server is crashed.
      // So. If contents size is empty.
      // Contents size will be replaced to screen size.
...
void DesktopWindowTreeHostOzone::WebAppWindowHidden() {
  // Remove an wayland window, but keep a renderer alive.
  Close();
}
