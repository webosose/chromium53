...
#if defined(OS_WEBOS)
void WaylandDisplay::CompositorBuffersSwapped(unsigned handle) {
  Dispatch(new Compositor_BuffersSwapped(handle));
}
...
void WaylandDisplay::KeyboardModifier(uint32_t mods_depressed,
                                      uint32_t mods_locked) {
  Dispatch(new WaylandInput_KeyboardModifier(mods_depressed, mods_locked));
}
...
void WaylandDisplay::InputPanelStateChanged(unsigned handle,
  ...
  Dispatch(new WaylandInput_InputPanelStateChanged(handle, state));
}
...
void WaylandDisplay::NativeWindowStateChanged(unsigned handle,
                                              ui::WidgetState state) {
  Dispatch(new WaylandWindow_StateChanged(handle, state));
}
...
void WaylandDisplay::TextInputModifier(uint32_t state, uint32_t modifier) {
  Dispatch(new WaylandInput_TextInputModifier(state, modifier));
}

void WaylandDisplay::SetKeyMask(unsigned w, uint32_t key_mask, bool value) {
  WaylandWindow* widget = GetWidget(w);
  if (!widget) {
    LOG(ERROR) << __PRETTY_FUNCTION__ << ", window not found.";
    return;
  }
  widget->SetKeyMask(static_cast<webos::WebOSKeyMask>(key_mask), value);
}

WaylandSeat* WaylandDisplay::SetPrimarySeat(wl_pointer* input_pointer) {
  for (auto seat : seat_list_) {
    if (seat->GetPointer()->Cursor()->GetInputPointer() == input_pointer) {
      primary_seat_ = seat;
      break;
    }
  }
  return primary_seat_;
}
...
void WaylandDisplay::WebOSXInputActivate(const std::string& type) {
  if (!webos_xinput_)
    return;
...
void WaylandDisplay::WebOSXInputDeactivate() {
  if (!webos_xinput_)
    return;
...
void WaylandDisplay::WebOSXInputInvokeAction(
    uint32_t keysym,
  ...
  switch (symbol_type) {
    case ui::WEBOS_XINPUT_QT_KEY_SYMBOL:
      wl_keysym_type = WL_WEBOS_XINPUT_KEYSYM_TYPE_QT;
      break;
    case ui::WEBOS_XINPUT_NATIVE_KEY_SYMBOL:
...
void WaylandDisplay::WindowClose(unsigned windowhandle) {
  Dispatch(new WaylandWindow_Close(windowhandle));
}
...
wl_webos_input_manager* WaylandDisplay::GetWebosInputManager() const {
  return webos_input_manager_;
}
...
intptr_t WaylandDisplay::GetNativeWindow(unsigned window_handle) {
  ...
  RAW_PMLOG_INFO("WebOSWebView",
                 "GetNativeWindow(id:%d widget:%p egl:%p) at %s(%d)",
                 window_handle, widget, widget ? widget->egl_window() : 0,
...
WaylandWindow* WaylandDisplay::CreateAcceleratedSurface(unsigned w) {
  ...
  RAW_PMLOG_INFO("WebOSWebView",
                 "Wayland Window(id:%d widget:%p) is created. at %s(%d)", w,
...
void WaylandDisplay::InitWindow(unsigned handle,
  ...
  switch (type) {
  case ui::WINDOW:
    window->SetShellAttributes(WaylandWindow::FULLSCREEN);
    break;
...
void WaylandDisplay::ShowInputPanel(unsigned widget) {
  primary_seat_->ShowInputPanel(widget);
}
...
void WaylandDisplay::SetInputContentType(ui::InputContentType content_type,
  ...
  primary_seat_->SetInputContentType(content_type, text_input_flags);
}
...
void WaylandDisplay::DisplayHandleGlobal(void *data,
  ...
  if (strcmp(interface, "wl_compositor") == 0) {
    ...
    static const struct wl_webos_input_manager_listener kWebosInputManagerListener = {
    ...
    disp->group_compositor_.reset(new WebOSSurfaceGroupCompositor(registry, name));
  }
#else
...
bool WaylandDisplay::OnMessageReceived(const IPC::Message& message) {
...
#if defined(OS_WEBOS)
  IPC_MESSAGE_HANDLER(WaylandDisplay_KeyMask, SetKeyMask)
  IPC_MESSAGE_HANDLER(WaylandDisplay_Property, SetWindowProperty)
  ...
  IPC_MESSAGE_HANDLER(WaylandDisplay_WebOSXInputActivate, WebOSXInputActivate)
  IPC_MESSAGE_HANDLER(WaylandDisplay_WebOSXInputInvokeAction,
                      WebOSXInputInvokeAction)
  IPC_MESSAGE_HANDLER(WaylandDisplay_WebOSXInputDeactivate,
                      WebOSXInputDeactivate)
#endif
...
void WaylandDisplay::HideIme(unsigned handle, ui::ImeHiddenType hidden_type) {
  Dispatch(new WaylandInput_HideIme(handle, hidden_type));
}
...
void WaylandDisplay::Dispatch(IPC::Message* message) {
  ...
  loop_->task_runner()->PostTask(
      FROM_HERE, base::Bind(&WaylandDisplay::Send, weak_ptr_, message));
}
