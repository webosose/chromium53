...
class GLHelper::CopyTextureToImpl
 ...
 private:
  ...
  GLuint ScaleTexture(GLuint src_texture,
                      ...
                      bool vertically_flip_texture,
                      bool swizzle,
                      GLenum dst_texture_format,
                      GLenum dst_texture_type,
                      GLHelper::ScalerQuality quality);
...
GLuint GLHelper::CopyTextureToImpl::ScaleTexture(
  ...
  gl_->GenTextures(1, &dst_texture);
  {
    ...
    gl_->TexImage2D(GL_TEXTURE_2D, 0, dst_texture_format, dst_size.width(),
                    dst_size.height(), 0, dst_texture_format, dst_texture_type, NULL);
  }
...
void GLHelper::CopyTextureToImpl::CropScaleReadbackAndCleanTexture(
  ...
  if (scale_texture) {
    ...
    bool scale_swizzle = out_color_type == kAlpha_8_SkColorType
                             ...
                             GLenum dst_texture_format = GL_RGBA;
    GLenum dst_texture_type = GL_UNSIGNED_BYTE;
    // Use GL_RGBA for destination/temporary texture unless we're working with
    // 16-bit data
    if (out_color_type == kRGB_565_SkColorType) {
      dst_texture_format = GL_RGB;
      dst_texture_type = GL_UNSIGNED_SHORT_5_6_5;
    }
    if (format == GL_BGRA_EXT &&
        supported == GLHelperReadbackSupport::SUPPORTED)
      dst_texture_format = GL_BGRA_EXT;
    texture = ScaleTexture(src_texture, src_size, src_subrect, dst_size, true,
                           scale_swizzle, dst_texture_format, dst_texture_type,
                           quality);
    DCHECK(texture);
  }
