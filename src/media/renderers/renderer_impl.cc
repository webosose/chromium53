...
void RendererImpl::SetPlaybackRate(double playback_rate) {
  ...
  // For starting Broadcom's HW decoder before feed input data,
  // set |time_ticking_| to true for calling media_apis_wrapper->SetPlaybackRate.
  if (media_apis_wrapper_ && playback_rate > 0) {
      time_ticking_ = true;
  ...
  if (media_apis_wrapper_)
    media_apis_wrapper_->SetPlaybackRate(playback_rate);
#endif
}
...
base::TimeDelta RendererImpl::GetMediaTime() {
  ...
  // threads.
  return time_source_ ? time_source_->CurrentMediaTime() : base::TimeDelta();
}
...
#if defined(OS_WEBOS) && defined(USE_UMEDIASERVER)
void RendererImpl::SetMediaAPIsWrapper(
...
#if defined(OS_WEBOS) && defined(USE_UMEDIASERVER)
  if (media_apis_wrapper_)
    media_apis_wrapper_->SetPlaybackRate(0.0f);
#endif
}
...
#if defined(OS_WEBOS) && defined(USE_UMEDIASERVER)
  if (media_apis_wrapper_)
    media_apis_wrapper_->SetPlaybackRate(playback_rate_);
#endif
}
