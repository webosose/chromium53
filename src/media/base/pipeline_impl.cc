...
#if defined(OS_WEBOS) && defined(USE_UMEDIASERVER)
  void DoSeekForMediaAPIsWrapper(base::TimeDelta seek_timestamp);
#endif
...
#if defined(OS_WEBOS) && defined(USE_UMEDIASERVER)
void PipelineImpl::RendererWrapper::DoSeekForMediaAPIsWrapper(
    base::TimeDelta seek_timestamp) {
  RAW_PMLOG_DEBUG("%s - %u", __FUNCTION__, seek_timestamp.InMilliseconds());

  auto pipeline = weak_pipeline_.get();

  if (pipeline && pipeline->media_apis_wrapper_) {
    if (!pipeline->media_apis_wrapper_->Seek(seek_timestamp))
      RAW_PMLOG_DEBUG("media_apis_wrapper_->Seek Failed!!");
  }
}
#endif
...
#if defined(OS_WEBOS) && defined(USE_UMEDIASERVER)
  // Seek in media_apis_wrapper_ (just causes queued buffers to be flushed)
  bound_fns.Push(
      base::Bind(&PipelineImpl::RendererWrapper::DoSeekForMediaAPIsWrapper,
...
#if defined(OS_WEBOS) && defined(USE_UMEDIASERVER)
void PipelineImpl::SetMediaAPIsWrapper(
    scoped_refptr<MediaAPIsWrapper>& media_apis_wrapper) {
...
void PipelineImpl::RendererWrapper::StateTransitionTask(PipelineStatus status) {
...
#if defined(OS_WEBOS) && defined(USE_UMEDIASERVER)
  auto pipeline = weak_pipeline_.get();
  RAW_PMLOG_DEBUG("Pipeline::StateTransitionTask - state : %s",
  ...
  switch (state_) {
    ...
    case kInitMediaApi:
      if (pipeline)
        pipeline->InitializeMediaApi(done_cb);
      return;
#endif
  }
}
...
#if defined(OS_WEBOS) && defined(USE_UMEDIASERVER)
void PipelineImpl::InitializeMediaApi(const PipelineStatusCB& done_cb) {
  DCHECK(media_task_runner_->BelongsToCurrentThread());

  auto renderer_wrapper = renderer_wrapper_.get();
  if (!media_apis_wrapper_ || !renderer_wrapper)
    return;

  PipelineStatusCB done_cb_ = done_cb;
  VideoDecoderConfig video_config;
  AudioDecoderConfig audio_config;

  DemuxerStream* video_stream = renderer_wrapper->demuxer_->GetStream(DemuxerStream::VIDEO);
  DemuxerStream* audio_stream = renderer_wrapper->demuxer_->GetStream(DemuxerStream::AUDIO);

  if (!video_stream && !audio_stream)
      return base::ResetAndReturn(&done_cb_).Run(PIPELINE_OK);

  {
    base::AutoLock l(renderer_wrapper->shared_state_lock_);
    if (video_stream)
  ...
  if (video_stream)
#if defined(ENABLE_VP9)
    if (video_config.codec() != kCodecH264 && video_config.codec() != kCodecVP9)
      return base::ResetAndReturn(&done_cb_).Run(PIPELINE_OK);
#else
    if (video_config.codec() != kCodecH264)
      return base::ResetAndReturn(&done_cb_).Run(PIPELINE_OK);
#endif

  media_apis_wrapper_->Initialize(
      audio_config,
      video_config,
      done_cb);
}
