...
#ifndef MEDIA_TV_RENDERERS_VIDEO_RENDERER_H_
#define MEDIA_TV_RENDERERS_VIDEO_RENDERER_H_
...
// TvVideoRenderer handles reading from a VideoFrameStream storing the
// results in a queue of decoded frames and executing a callback when a frame is
// ready for rendering.
class MEDIA_EXPORT TvVideoRenderer
 ...
 private:
  ...
  // Sink which calls into TvVideoRenderer via Render() for video frames.  Do
  ...
  // Provides video frames to TvVideoRenderer.
  ...
  base::WeakPtrFactory<TvVideoRenderer> weak_factory_;
  ...
  // been reset.
  base::WeakPtrFactory<TvVideoRenderer> frame_callback_weak_factory_;

  DISALLOW_COPY_AND_ASSIGN(TvVideoRenderer);
};

}  // namespace media

#endif  // MEDIA_TV_RENDERERS_VIDEO_RENDERER_H_
