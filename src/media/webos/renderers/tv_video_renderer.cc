...
TvVideoRenderer::TvVideoRenderer(
...
void TvVideoRenderer::Flush(const base::Closure& callback) {
  ...
  if (buffering_state_ != BUFFERING_HAVE_NOTHING) {
    ...
    task_runner_->PostTask(
        FROM_HERE, base::Bind(&TvVideoRenderer::OnBufferingStateChange,
  ...
  video_frame_stream_->Reset(
      base::Bind(&TvVideoRenderer::OnVideoFrameStreamResetDone,
                 weak_factory_.GetWeakPtr()));
}

void TvVideoRenderer::StartPlayingFrom(base::TimeDelta timestamp) {
...
void TvVideoRenderer::Initialize(
  ...
  video_frame_stream_->Initialize(
      stream, base::Bind(&TvVideoRenderer::OnVideoFrameStreamInitialized,
                         weak_factory_.GetWeakPtr()),
      cdm_context, base::Bind(&TvVideoRenderer::OnStatisticsUpdate,
                              weak_factory_.GetWeakPtr()),
...
scoped_refptr<VideoFrame> TvVideoRenderer::Render(
  ...
  if (buffering_state_ == BUFFERING_HAVE_ENOUGH && !received_end_of_stream_ &&
    ...
    task_runner_->PostTask(
        FROM_HERE, base::Bind(&TvVideoRenderer::TransitionToHaveNothing,
                              weak_factory_.GetWeakPtr()));
  }
  ...
  task_runner_->PostTask(
      FROM_HERE,
      base::Bind(&TvVideoRenderer::AttemptReadAndCheckForMetadataChanges,
...
void TvVideoRenderer::OnFrameDropped() {
  base::AutoLock auto_lock(lock_);
  algorithm_->OnLastFrameDropped();
}

void TvVideoRenderer::OnVideoFrameStreamInitialized(bool success) {
  DCHECK(task_runner_->BelongsToCurrentThread());
  base::AutoLock auto_lock(lock_);
...
void TvVideoRenderer::OnPlaybackError(PipelineStatus error) {
  DCHECK(task_runner_->BelongsToCurrentThread());
  client_->OnError(error);
}

void TvVideoRenderer::OnPlaybackEnded() {
  DCHECK(task_runner_->BelongsToCurrentThread());
  client_->OnEnded();
}

void TvVideoRenderer::OnStatisticsUpdate(const PipelineStatistics& stats) {
  DCHECK(task_runner_->BelongsToCurrentThread());
  client_->OnStatisticsUpdate(stats);
}

void TvVideoRenderer::OnBufferingStateChange(BufferingState state) {
  DCHECK(task_runner_->BelongsToCurrentThread());
...
void TvVideoRenderer::FrameReadyForCopyingToGpuMemoryBuffers(
    VideoFrameStream::Status status,
    const scoped_refptr<VideoFrame>& frame) {
  if (status != VideoFrameStream::OK || IsBeforeStartTime(frame->timestamp())) {
    TvVideoRenderer::FrameReady(status, frame);
    return;
  }

  DCHECK(frame);
  gpu_memory_buffer_pool_->MaybeCreateHardwareFrame(
      frame, base::Bind(&TvVideoRenderer::FrameReady,
                        frame_callback_weak_factory_.GetWeakPtr(), status));
}

void TvVideoRenderer::FrameReady(VideoFrameStream::Status status,
                                   const scoped_refptr<VideoFrame>& frame) {
  DCHECK(task_runner_->BelongsToCurrentThread());
  base::AutoLock auto_lock(lock_);
  ...
  if (status == VideoFrameStream::DECODE_ERROR) {
    ...
    task_runner_->PostTask(
        FROM_HERE,
        base::Bind(&TvVideoRenderer::OnPlaybackError,
...
bool TvVideoRenderer::HaveEnoughData_Locked() {
  DCHECK_EQ(state_, kPlaying);
...
void TvVideoRenderer::TransitionToHaveEnough_Locked() {
  DCHECK(task_runner_->BelongsToCurrentThread());
  DCHECK_EQ(buffering_state_, BUFFERING_HAVE_NOTHING);
  lock_.AssertAcquired();
...
void TvVideoRenderer::TransitionToHaveNothing() {
  DCHECK(task_runner_->BelongsToCurrentThread());
...
void TvVideoRenderer::AddReadyFrame_Locked(
    const scoped_refptr<VideoFrame>& frame) {
  DCHECK(task_runner_->BelongsToCurrentThread());
  lock_.AssertAcquired();
...
void TvVideoRenderer::AttemptRead_Locked() {
  DCHECK(task_runner_->BelongsToCurrentThread());
  lock_.AssertAcquired();
    ...
    case kPlaying:
      ...
      if (gpu_memory_buffer_pool_) {
        video_frame_stream_->Read(base::Bind(
            &TvVideoRenderer::FrameReadyForCopyingToGpuMemoryBuffers,
            frame_callback_weak_factory_.GetWeakPtr()));
      } else {
        video_frame_stream_->Read(
            base::Bind(&TvVideoRenderer::FrameReady,
                       frame_callback_weak_factory_.GetWeakPtr()));
      }
      return;
    case kUninitialized:
...
void TvVideoRenderer::OnVideoFrameStreamResetDone() {
...
void TvVideoRenderer::UpdateStats_Locked() {
  lock_.AssertAcquired();
  ...
  if (frames_decoded_ || frames_dropped_) {
    ...
    task_runner_->PostTask(FROM_HERE,
                           base::Bind(&TvVideoRenderer::OnStatisticsUpdate,
...
bool TvVideoRenderer::HaveReachedBufferingCap() {
  DCHECK(task_runner_->BelongsToCurrentThread());
...
void TvVideoRenderer::StartSink() {
  DCHECK(task_runner_->BelongsToCurrentThread());
...
void TvVideoRenderer::MaybeFireEndedCallback_Locked(bool time_progressing) {
  lock_.AssertAcquired();
  ...
  if (!media_apis_wrapper_->IsEOSReceived())
    return;
  ...
  if (!algorithm_->effective_frames_queued() ||
    ...
    task_runner_->PostTask(FROM_HERE,
                           base::Bind(&TvVideoRenderer::OnPlaybackEnded,
                                      weak_factory_.GetWeakPtr()));
  }
}

base::TimeTicks TvVideoRenderer::ConvertMediaTimestamp(
...
base::TimeTicks TvVideoRenderer::GetCurrentMediaTimeAsWallClockTime() {
  std::vector<base::TimeTicks> current_time;
  wall_clock_time_cb_.Run(std::vector<base::TimeDelta>(), &current_time);
  return current_time[0];
}

bool TvVideoRenderer::IsBeforeStartTime(base::TimeDelta timestamp) {
  return timestamp + video_frame_stream_->AverageDuration() < start_timestamp_;
}

void TvVideoRenderer::RemoveFramesForUnderflowOrBackgroundRendering() {
...
void TvVideoRenderer::CheckForMetadataChanges(VideoPixelFormat pixel_format,
...
void TvVideoRenderer::AttemptReadAndCheckForMetadataChanges(
