...
#if defined(ENABLE_LG_SVP)
static scoped_refptr<DecoderBuffer> MakeSvpInBandHeader(
  ...
  if (!inband_serializer.CanDoSerialize()) {
    RAW_PMLOG_DEBUG("DecryptingDemuxerStream",
                    "inband_serializer for <bool> is not supported");
    return NULL;
  }

  if (!inband_serializer.Configure(false, buffer->data(), buffer->data_size())) {
    RAW_PMLOG_DEBUG("DecryptingDemuxerStream",
                    "Failed to inband_serializer.Configure(true)");
    return NULL;
  }

  if (!inband_serializer.GetSerialized(&serialized_data)) {
    RAW_PMLOG_DEBUG("DecryptingDemuxerStream",
                    "Failed to inband_serializer.GetSerialized()");
    return NULL;
  }

  scoped_refptr<DecoderBuffer> out_buffer = DecoderBuffer::CopyFrom(
...
#if defined(WIDEVINE_CDM_AVAILABLE)
    if (decryptor_->KeySystem() == kWidevineKeySystem)
      use_secure_buffer = true;
#endif

#if defined(ENABLE_PLAYREADY_CDM)
    if (decryptor_->KeySystem() == kPlayReadyMicrosoftKeySystem
        || decryptor_->KeySystem() == kPlayReadyYouTubeKeySystem)
      use_secure_buffer = false;
#endif

  // An audio stream doesn't use secure buffer.
    if (demuxer_stream_->type() == AUDIO)
      use_secure_buffer = false;

    if (use_secure_buffer) {
      scoped_refptr<DecoderBuffer> decrypted = MakeSvpInBandHeader(buffer);
      state_ = kIdle;
...
void DecryptingDemuxerStream::DeliverBuffer(
  ...
  // set the duration from the encrypted to decrypted buffer
  if (decrypted_buffer->duration() == base::TimeDelta::FromMicroseconds(0)) {
