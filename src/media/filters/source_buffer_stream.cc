...
SourceBufferStream::SourceBufferStream(const AudioDecoderConfig& audio_config,
  ...
  int memory_limit = 0;
  std::string memory_limit_str(base::CommandLine::ForCurrentProcess()->GetSwitchValueASCII(
      switches::kMaxAudioSourceBuffer));
  if (base::StringToInt(memory_limit_str, &memory_limit) && memory_limit > 0)
    memory_limit_ = memory_limit * kMegabyte;
}
...
SourceBufferStream::SourceBufferStream(const VideoDecoderConfig& video_config,
  ...
  int memory_limit = 0;
  std::string memory_limit_str(base::CommandLine::ForCurrentProcess()->GetSwitchValueASCII(
      switches::kMaxVideoSourceBuffer));
  if (base::StringToInt(memory_limit_str, &memory_limit) && memory_limit > 0)
    memory_limit_ = memory_limit * kMegabyte;
}
