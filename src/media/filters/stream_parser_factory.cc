...
#endif
#if defined(OS_WEBOS) && defined(USE_UMEDIASERVER)
  Json::Value supported_codec_mse = media::MediaAPIsWrapper::SupportedCodecMSE();
#endif
  switch (codec_info->type) {
    case CodecInfo::AUDIO:
#if defined(OS_WEBOS) && defined(USE_UMEDIASERVER)
      if (codec_info->tag == CodecInfo::HISTOGRAM_VORBIS ||
          codec_info->tag == CodecInfo::HISTOGRAM_OPUS)
        return false;
      if (codec_info->tag == CodecInfo::HISTOGRAM_MPEG4AAC ||
          codec_info->tag == CodecInfo::HISTOGRAM_MPEG2AAC) {
        if (!channels.empty() &&
            (std::stoi(channels) > supported_codec_mse["AAC"]["channels"].asInt())) {
          return false;
        }
      }
#endif
    ...
    case CodecInfo::VIDEO:
      ...
      if (codec_info->tag == CodecInfo::HISTOGRAM_VP9) {
        ...
        int64_t vp9_max_bitrate_64 = supported_codec_mse["VP9"]["maxBitRate"].asInt() * 1024 * 1024;
        if ((!width.empty() &&
              (std::stoi(width) > supported_codec_mse["VP9"]["maxWidth"].asInt())) ||
            (!height.empty() &&
              (std::stoi(height) > supported_codec_mse["VP9"]["maxHeight"].asInt())) ||
            (!frameRate.empty() &&
              (std::stoi(frameRate) > supported_codec_mse["VP9"]["maxFrameRate"].asInt())) ||
            (!bitRate.empty() &&
              (std::stoi(bitRate) > vp9_max_bitrate_64)))
          return false;
#else
        return false;
#endif
      } else if (codec_info->tag == CodecInfo::HISTOGRAM_H264) {
        int64_t h264_max_bitrate_64 = supported_codec_mse["H264"]["maxBitRate"].asInt() * 1024 * 1024;
        if ((!width.empty() &&
              (std::stoi(width) > supported_codec_mse["H264"]["maxWidth"].asInt())) ||
            (!height.empty() &&
              (std::stoi(height) > supported_codec_mse["H264"]["maxHeight"].asInt())) ||
            (!frameRate.empty() &&
              (std::stoi(frameRate) > supported_codec_mse["H264"]["maxFrameRate"].asInt())) ||
            (!bitRate.empty() &&
              (std::stoi(bitRate) > h264_max_bitrate_64)))
          return false;
      }
#endif
      if (video_codecs)
...
#if defined(OS_WEBOS) && defined(USE_UMEDIASERVER)
  return CheckTypeAndCodecs(type, codecs,
                            "", "", "", "", "",
                            new MediaLog(), NULL, NULL, NULL);
#else
...
#if defined(OS_WEBOS) && defined(USE_UMEDIASERVER)
  if (CheckTypeAndCodecs(type, codecs,
                         "", "", "", "", "",
                         media_log,
                         &factory_function,
                         &audio_codecs,
