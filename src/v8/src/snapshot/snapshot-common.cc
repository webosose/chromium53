...
int SnapshotData::compress(const byte* data, const int length,
                           byte* compressed) {
  base::ElapsedTimer timer;
  if (FLAG_profile_deserialization) timer.Start();
  ...
  snappy::RawCompress(source, length, dest, &compressedLength);
...
SnapshotData::SnapshotData(const Serializer* serializer) {
  ...
  if (FLAG_compress_startup_blob) {
    int compressed_size = compress(payload->begin(), payload->length(),
    ...
    SetHeaderValue(kCompressedLengthOffset, payload->length());
  }
}
