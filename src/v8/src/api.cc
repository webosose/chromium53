...
void ReportException(TryCatch* try_catch) {
  ...
  if (message.IsEmpty()) {
    ...
    i::PrintF("%s:%i: %s\n",
              ToCString(filename_value), message->GetLineNumber(), exception);
...
#if V8_OS_ANDROID || defined(OS_WEBOS)
  // Android has higher physical memory requirements before raising the maximum
  // heap size limits since it has no swap space.
  const uint64_t ultralow_limit = 384ul * i::MB;
...
#if defined(OS_WEBOS)
  // Calculate on TV-like platforms a part of the physical memory will be
  // actually needed for media and GPU buffering
  physical_memory = physical_memory * 3 / 4;
#endif

  if (physical_memory <= ultralow_limit) {
    set_max_semi_space_size(i::Heap::kMaxSemiSpaceSizeUltraLowMemoryDevice);
    set_max_old_space_size(i::Heap::kMaxOldSpaceSizeUltraLowMemoryDevice);
    set_max_executable_size(i::Heap::kMaxExecutableSizeUltraLowMemoryDevice);
  } else if (physical_memory <= low_limit) {
...
void ResourceConstraints::ConfigureDetails(size_t min_allocation_limit_growing_step_size,
                                           size_t high_fragmentation_slack,
                                           int external_allocation_hard_limit,
                                           int external_allocation_soft_limit) {
  set_min_allocation_limit_growing_step_size(min_allocation_limit_growing_step_size);
...
void SetResourceConstraints(i::Isolate* isolate,
  ...
  if (semi_space_size != 0 || old_space_size != 0 ||
    ...
    isolate->heap()->ConfigureHeapDetails(constraints.min_allocation_limit_growing_step_size(),
...
bool Script::CompileExtension(Isolate* isolate, v8::Local<String> source,
  ...
  i::Vector<const char> name = i::CStrVector(extension->name());
  i::SourceCodeCache* cache = i_isolate->bootstrapper()->extensions_cache();
  i::Handle<i::Context> context(i_isolate->context());
  DCHECK(context->IsNativeContext());

  i::Handle<i::String> script_name = factory->NewStringFromUtf8(name).ToHandleChecked();
  function_info = i::Compiler::GetSharedFunctionInfoForScript(
      str, script_name, 0, 0, ScriptOriginOptions(), i::Handle<i::Object>(),
  ...
  return !i::Execution::TryCall(i_isolate, fun, receiver, 0, nullptr,
                             i::Execution::MessageHandling::kKeepPending, nullptr)
              .is_null();
}
...
void Isolate::SetLaunchOptimization(bool enabled) {
  i::Isolate* isolate = reinterpret_cast<i::Isolate*>(this);
  isolate->SetLaunchState(enabled? kLaunching : kLaunched);
}
