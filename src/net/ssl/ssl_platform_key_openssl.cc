...
class SSLPlatformKeyOpenSSL : public ThreadedSSLPrivateKey::Delegate {
 public:
  SSLPlatformKeyOpenSSL(crypto::ScopedEVP_PKEY key, SSLPrivateKey::Type type)
      : key_(std::move(key)), type_(type) {}

  ~SSLPlatformKeyOpenSSL() override {}
 ...
 private:
  ...
  DISALLOW_COPY_AND_ASSIGN(SSLPlatformKeyOpenSSL);
};
...
scoped_refptr<SSLPrivateKey> WrapOpenSSLPrivateKey(crypto::ScopedEVP_PKEY key) {
  ...
  return make_scoped_refptr(new ThreadedSSLPrivateKey(
      base::WrapUnique(new SSLPlatformKeyOpenSSL(std::move(key), type)),
