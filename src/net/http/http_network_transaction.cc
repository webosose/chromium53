...
int HttpNetworkTransaction::DoSendRequest() {
  ...
  if (IsEnableNetworkLog()) {
    RAW_PMLOG_DEBUG("> %s %s",request_->method.c_str(), request_->url.spec().c_str());
    if (!request_headers_.IsEmpty()) {
      HttpRequestHeaders::Iterator ite(request_headers_);
      do {
        RAW_PMLOG_DEBUG("> %s: %s", ite.name().c_str(), ite.value().c_str());
      } while (ite.GetNext());
    }
    RAW_PMLOG_DEBUG("> ");
  }
...
int HttpNetworkTransaction::DoReadHeadersComplete(int result) {
  ...
  if (IsEnableNetworkLog()) {
    RAW_PMLOG_DEBUG("< %s", response_.headers->GetStatusLine().c_str());
    size_t iter = 0;
    std::string name;
    std::string value;
    while (response_.headers->EnumerateHeaderLines(&iter, &name, &value)) {
      RAW_PMLOG_DEBUG("< %s: %s", name.c_str(), value.c_str());
    }
    RAW_PMLOG_DEBUG("< ");
  }
...
bool HttpNetworkTransaction::IsEnableNetworkLog() {
