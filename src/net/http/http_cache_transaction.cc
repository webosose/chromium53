...
int HttpCache::Transaction::DoCacheReadResponseComplete(int result) {
  ...
  if ((next_state_ != STATE_SEND_REQUEST) && IsEnableNetworkLog()) {
    RAW_PMLOG_DEBUG("C> %s %s",request_->method.c_str(), request_->url.spec().c_str());
    if (!request_->extra_headers.IsEmpty()) {
      HttpRequestHeaders::Iterator ite(request_->extra_headers);
      do {
        RAW_PMLOG_DEBUG("C> %s: %s", ite.name().c_str(), ite.value().c_str());
      } while (ite.GetNext());
    }
    RAW_PMLOG_DEBUG("C> ");

    RAW_PMLOG_DEBUG("C< %s", response_.headers->GetStatusLine().c_str());
    size_t iter = 0;
    std::string name;
    std::string value;
    while (response_.headers->EnumerateHeaderLines(&iter, &name, &value)) {
      RAW_PMLOG_DEBUG("C< %s: %s", name.c_str(), value.c_str());
    }
    RAW_PMLOG_DEBUG("C< ");
  }
...
bool HttpCache::Transaction::IsEnableNetworkLog() {
  ...
  std::string value = base::CommandLine::ForCurrentProcess()->GetSwitchValueASCII("webos-log");
  if (!value.empty() && strcasestr(value.c_str(),"Network")) {
    enable = true;
  }
  init = true;
