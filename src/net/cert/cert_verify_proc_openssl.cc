...
int CertVerifyProcOpenSSL::VerifyInternal(
  ...
  if (X509_verify_cert(ctx.get()) != 1) {
    ...
    time_t raw_time;
    time(&raw_time);
    RAW_PMLOG_INFO("Network", "%s(%d) : depth(%d) : status(%d) : date(%s)",
        X509_verify_cert_error_string(x509_error), x509_error, X509_STORE_CTX_get_error_depth(ctx.get()), cert_status, ctime(&raw_time));
